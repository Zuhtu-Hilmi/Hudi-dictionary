const dictionary = {
    "a": "bir",
    "abandon": "terk etmek",
    "abandoned": "terkedilmiş",
    "ability": "kabiliyet",
    "able": "yapabilmek",
    "about": "hakkında",
    "above": "yukarıda",
    "abroad": "yurt dışı",
    "absence": "yokluk",
    "absent": "yok",
    "absolute": "kesin",
    "absolutely": "kesinlikle",
    "absorb": "emmek",
    "abuse": "taciz",
    "academic": "akademik",
    "accent": "Aksan",
    "accept": "kabul etmek",
    "acceptable": "kabul edilebilir",
    "access": "Giriş",
    "accident": "kaza",
    "accidental": "tesadüfi",
    "accidentally": "kazara",
    "accommodation": "Konaklama",
    "accompany": "eşlik etmek",
    "according to": "göre",
    "account": "hesap",
    "account for": "hesap vermek",
    "accurate": "doğru",
    "accurately": "tam olarak",
    "accuse": "suçlamak",
    "achieve": "başarmak",
    "achievement": "kazanım",
    "acid": "asit",
    "acknowledge": "tanındıktan",
    "a couple": "bir çift",
    "acquire": "elde etmek",
    "across": "karşısında",
    "act": "davranmak",
    "action": "aksiyon",
    "active": "aktif",
    "actively": "aktif",
    "activity": "aktivite",
    "actor": "aktör",
    "actress": "aktris",
    "actual": "gerçek",
    "actually": "aslında",
    "ad": "ilan",
    "adapt": "adapte olmak",
    "add": "Ekle",
    "addition": "ilave",
    "additional": "ek",
    "add on": "Ayriyeten",
    "address": "adres",
    "add up": "toplamak",
    "add up to": "ekleyebilirsiniz",
    "adequate": "yeterli",
    "adequately": "yeterli olarak",
    "adjust": "ayarlamak",
    "admiration": "hayranlık",
    "admire": "beğenmek",
    "admit": "Kabul et",
    "adopt": "evlat edinmek",
    "adult": "yetişkin",
    "advance": "ilerlemek",
    "advanced": "ileri",
    "advantage": "avantaj",
    "adventure": "macera",
    "advert": "ilan",
    "advertise": "duyurmak",
    "advertisement": "İlan",
    "advertising": "reklâm",
    "advice": "tavsiye",
    "advise": "öğüt vermek",
    "affair": "mesele",
    "affect": "etkilemek",
    "affection": "sevgi",
    "afford": "parası yetmek",
    "afraid": "korkmuş",
    "after": "sonra",
    "afternoon": "öğleden sonra",
    "afterwards": "sonradan",
    "again": "tekrar",
    "against": "karşısında",
    "age": "yaş",
    "aged": "yaşlı",
    "agency": "Ajans",
    "agent": "ajan",
    "aggressive": "agresif",
    "ago": "önce",
    "agree": "Katılıyorum",
    "agreement": "anlaşma",
    "ahead": "önde",
    "aid": "yardım",
    "aim": "amaç",
    "air": "hava",
    "aircraft": "uçak",
    "airport": "havalimanı",
    "alarm": "alarm",
    "alarmed": "alarmlı",
    "alarming": "korkutucu",
    "alcohol": "alkol",
    "alcoholic": "alkollü",
    "alive": "canlı",
    "all": "her şey",
    "allied": "müttefik",
    "allow": "izin vermek",
    "allow for": "izin",
    "all right": "tamam",
    "ally": "müttefik",
    "almost": "neredeyse",
    "alone": "tek başına",
    "along": "boyunca",
    "alongside": "yanında",
    "aloud": "yüksek sesle",
    "alphabet": "alfabe",
    "alphabetical": "alfabetik",
    "alphabetically": "alfabetik olarak",
    "already": "zaten",
    "also": "Ayrıca",
    "alter": "alter",
    "alternative": "alternatif",
    "alternatively": "alternatif olarak",
    "although": "olmasına rağmen",
    "altogether": "tamamen",
    "always": "her zaman",
    "a.m.": "sabah",
    "amaze": "Amaze",
    "amazed": "şaşırmış",
    "amazing": "inanılmaz",
    "ambition": "hırs",
    "ambulance": "ambulans",
    "among": "arasında",
    "amount": "Miktar",
    "amount to": "tutarı",
    "amuse": "eğlendirmek",
    "amused": "eğlenerek",
    "amusing": "eğlenceli",
    "analyse": "çözümlemek",
    "analysis": "analiz",
    "ancient": "Antik",
    "and": "ve",
    "anger": "öfke",
    "angle": "açı",
    "angrily": "öfkeyle",
    "angry": "kızgın",
    "animal": "hayvan",
    "ankle": "ayak bileği",
    "anniversary": "yıldönümü",
    "announce": "duyurmak",
    "annoy": "kızdırmak",
    "annoyed": "kızgın",
    "annoying": "Can sıkıcı",
    "annual": "yıllık",
    "annually": "yılda",
    "another": "bir diğeri",
    "answer": "Cevap",
    "anti-": "anti",
    "anticipate": "tahmin etmek",
    "anxiety": "kaygı",
    "anxious": "endişeli",
    "anxiously": "endişeyle",
    "any": "hiç",
    "anybody": "kimse",
    "anyone": "kimse",
    "anything": "herhangi bir şey",
    "anyway": "neyse",
    "anywhere": "herhangi bir yer",
    "apart": "ayrı",
    "apart from": "dışında",
    "apartment": "apartman",
    "apologize": "özür dilemek",
    "apparent": "bariz",
    "apparently": "görünüşe göre",
    "appeal": "temyiz",
    "appear": "görünmek",
    "appearance": "görünüm",
    "apple": "elma",
    "application": "uygulama",
    "apply": "uygulamak",
    "appoint": "atamak",
    "appointment": "randevu",
    "appreciate": "takdir etmek",
    "approach": "yaklaşmak",
    "appropriate": "uygun",
    "approval": "onay",
    "approve": "onaylamak",
    "approving": "onaylayan",
    "approximate": "yaklaşık",
    "approximately": "yaklaşık olarak",
    "April": "Nisan",
    "area": "alan",
    "argue": "tartışmak",
    "argument": "tartışma",
    "arise": "ortaya çıkmak",
    "arm": "kol",
    "armed": "silahlı",
    "arms": "silâh",
    "army": "Ordu",
    "around": "etrafında",
    "arrange": "düzenlemek",
    "arrangement": "aranjman",
    "arrest": "tutuklamak",
    "arrival": "varış",
    "arrive": "varmak",
    "arrive at": "ulaşmak",
    "arrow": "ok",
    "art": "Sanat",
    "article": "makale",
    "artificial": "yapay",
    "artificially": "yapay",
    "artist": "sanatçı",
    "artistic": "artistik",
    "artistically": "sanatsal",
    "as": "gibi",
    "ashamed": "utanmış",
    "aside": "bir kenara",
    "aside from": "den başka",
    "ask": "Sor",
    "asleep": "uykuda",
    "aspect": "Görünüş",
    "assist": "yardım",
    "assistance": "yardım",
    "assistant": "asistan",
    "associate": "ortak",
    "associated": "birleşmiş",
    "association": "bağlantı",
    "assume": "üstlenmek",
    "assure": "temin etmek",
    "at": "en",
    "atmosphere": "atmosfer",
    "atom": "atom",
    "attach": "iliştirmek",
    "attached": "ekli",
    "attack": "saldırı",
    "attempt": "girişim",
    "attempted": "teşebbüs",
    "attend": "düzenli gidenler",
    "attend to": "katılmak",
    "attention": "Dikkat",
    "attitude": "tutum",
    "attorney": "avukat",
    "attract": "çekmek",
    "attraction": "cazibe",
    "attractive": "çekici",
    "audience": "seyirci",
    "August": "Ağustos",
    "aunt": "teyze",
    "author": "yazar",
    "authority": "yetki",
    "automatic": "otomatik",
    "automatically": "otomatik olarak",
    "autumn": "sonbahar",
    "available": "mevcut",
    "average": "ortalama",
    "avoid": "önlemek",
    "awake": "uyanık",
    "award": "ödül",
    "aware": "farkında",
    "away": "uzakta",
    "awful": "korkunç",
    "awfully": "çok",
    "awkward": "garip",
    "awkwardly": "beceriksizce",
    "baby": "bebek",
    "back": "geri",
    "background": "arka fon",
    "back up": "destek olmak",
    "backward": "geriye",
    "backwards": "geriye doğru",
    "bacteria": "bakteriler",
    "bad": "kötü",
    "badly": "kötü",
    "bad-tempered": "kötü huylu",
    "bag": "sırt çantası",
    "baggage": "bagaj",
    "bake": "pişirmek",
    "balance": "denge",
    "ball": "top",
    "ban": "yasak",
    "band": "grup",
    "bandage": "bandaj",
    "bank": "banka",
    "bar": "bar",
    "bargain": "pazarlık etmek",
    "barrier": "bariyer",
    "base": "baz",
    "based": "merkezli",
    "base on": "üzerinde taban",
    "basic": "temel",
    "basically": "temel olarak",
    "basis": "temel",
    "bath": "banyo",
    "bathroom": "banyo",
    "battery": "pil",
    "battle": "savaş",
    "bay": "Defne",
    "be": "olmak",
    "beach": "plaj",
    "beak": "gaga",
    "bear": "ayı",
    "beard": "sakal",
    "beat": "dövmek",
    "beat up": "dövmek",
    "beautiful": "güzel",
    "beautifully": "güzelce",
    "beauty": "güzellik",
    "because": "Çünkü",
    "because of": "nedeniyle",
    "become": "olmak",
    "bed": "yatak",
    "bedroom": "yatak odası",
    "beef": "sığır eti",
    "beer": "bira",
    "before": "önce",
    "begin": "başla",
    "beginning": "başlangıç",
    "behalf": "adına",
    "behave": "Davranmak",
    "behaviour": "davranış",
    "behind": "arkasında",
    "be left over": "artmak",
    "belief": "inanç",
    "believe": "inanmak",
    "believe in": "inan",
    "bell": "çan",
    "belong": "ait olmak",
    "belong to": "ait olmak",
    "below": "altında",
    "belt": "kemer",
    "bend": "Bükmek",
    "beneath": "altında",
    "benefit": "yarar",
    "bent": "kıvrılmış",
    "beside": "yanında",
    "best": "en iyi",
    "bet": "bahis",
    "better": "daha iyi",
    "betting": "bahis",
    "between": "arasında",
    "beyond": "ötesinde",
    "bicycle": "bisiklet",
    "bid 1": "1 teklif",
    "big": "büyük",
    "bike": "bisiklet",
    "bill": "fatura",
    "billion": "milyar",
    "bin": "çöp Kutusu",
    "biology": "Biyoloji",
    "bird": "kuş",
    "birth": "doğum",
    "birthday": "doğum günü",
    "biscuit": "bisküvi",
    "bit": "bit",
    "bite": "ısırmak",
    "bitter": "acı",
    "bitterly": "acı",
    "black": "siyah",
    "blade": "bıçak ağzı",
    "blame": "suçlama",
    "blank": "boş",
    "blind": "kör",
    "block": "blok",
    "blonde": "sarışın",
    "blood": "kan",
    "blow": "darbe",
    "blow out": "üflemek",
    "blow up": "patlamak",
    "blue": "mavi",
    "board": "yazı tahtası",
    "boat": "tekne",
    "body": "vücut",
    "boil": "kaynama",
    "bomb": "bomba",
    "bone": "kemik",
    "book": "kitap",
    "boot": "çizme",
    "border": "sınır",
    "bore": "delik",
    "bored": "canı sıkkın",
    "boring": "sıkıcı",
    "born": "doğan",
    "borrow": "ödünç almak",
    "boss": "patron",
    "both": "her ikisi de",
    "bother": "zahmet",
    "bottle": "şişe",
    "bottom": "alt",
    "bound": "ciltli",
    "bowl": "kâse",
    "box": "Kutu",
    "boy": "oğlan",
    "boyfriend": "erkek arkadaş",
    "brain": "beyin",
    "branch": "şube",
    "brand": "marka",
    "brave": "cesur",
    "bread": "ekmek",
    "break": "mola",
    "break down": "Yıkmak",
    "breakfast": "kahvaltı",
    "break in": "sözünü kesmek",
    "break into": "zorla girmek",
    "break off": "kopmak",
    "break out": "çıkmak",
    "break up": "ayrılmak",
    "breast": "meme",
    "breath": "nefes",
    "breathe": "nefes almak",
    "breathe in": "nefes al",
    "breathe out": "nefes ver",
    "breathing": "nefes",
    "breed": "doğurmak",
    "brick": "tuğla",
    "bridge": "köprü",
    "brief": "kısa",
    "briefly": "kısaca",
    "bright": "parlak",
    "brightly": "parlak",
    "brilliant": "parlak",
    "bring": "getirmek",
    "bring back": "geri getir",
    "bring down": "indirmek",
    "bring forward": "öne sürmek",
    "bring out": "ortaya çıkarmak",
    "bring up": "konuyu açmak",
    "broad": "kalın",
    "broadcast": "yayın yapmak",
    "broadly": "geniş",
    "broken": "kırık",
    "brother": "erkek kardeş",
    "brown": "Kahverengi",
    "brush": "fırça",
    "bubble": "kabarcık",
    "budget": "bütçe",
    "build": "inşa etmek",
    "building": "bina",
    "build up": "kurmak",
    "bullet": "mermi",
    "bunch": "Demet",
    "burn": "yanmak",
    "burn down": "kül olmak",
    "burnt": "yanmış",
    "burst": "patlamak",
    "burst into": "boğulmak",
    "burst out": "haykırmak",
    "bury": "gömmek",
    "bus": "otobüs",
    "bush": "çalı",
    "business": "iş",
    "businessman": "işadamı",
    "busy": "meşgul",
    "but": "fakat",
    "butter": "Tereyağı",
    "button": "buton",
    "buy": "satın",
    "buyer": "alıcı",
    "by": "tarafından",
    "bye": "Hoşçakal",
    "cabinet": "kabine",
    "cable": "kablo",
    "cake": "kek",
    "calculate": "hesaplamak",
    "calculation": "hesaplama",
    "call": "aramak",
    "call back": "geri çağırmak",
    "called": "aranan",
    "call for": "çağırmak",
    "call off": "iptal",
    "call up": "çağırmak",
    "calm": "sakin",
    "calm down": "sakin ol",
    "calmly": "sakince",
    "camera": "kamera",
    "camp": "kamp",
    "campaign": "kampanya",
    "camping": "kamp yapmak",
    "can 1": "kutu 1",
    "can 2": "kutu 2",
    "cancel": "iptal etmek",
    "cancer": "kanser",
    "candidate": "aday",
    "candy": "Şeker",
    "cannot": "olumsuz",
    "cap": "kapak",
    "capable": "yetenekli",
    "capacity": "kapasite",
    "capital": "Başkent",
    "captain": "Kaptan",
    "capture": "ele geçirmek",
    "car": "araba",
    "card": "kart",
    "cardboard": "karton",
    "care": "bakım",
    "career": "kariyer",
    "care for": "bakmak",
    "careful": "dikkatli",
    "carefully": "dikkatlice",
    "careless": "dikkatsiz",
    "carelessly": "dikkatsizce",
    "carpet": "halı",
    "carrot": "havuç",
    "carry": "Taşımak",
    "carry on": "sürdürmek",
    "carry out": "gerçekleştirmek",
    "case": "durum",
    "cash": "nakit",
    "cast": "oyuncular",
    "castle": "kale",
    "cat": "kedi",
    "catch": "tutmak",
    "catch up": "yakalamak",
    "category": "kategori",
    "cause": "sebep olmak",
    "CD": "CD",
    "cease": "ateşkes",
    "ceiling": "tavan",
    "celebrate": "kutlamak",
    "celebration": "kutlama",
    "cell": "hücre",
    "cell phone": "cep telefonu",
    "cent": "sent",
    "centimetre": "santimetre",
    "central": "merkezi",
    "centre": "merkez",
    "century": "yüzyıl",
    "ceremony": "tören",
    "certain": "belirli",
    "certainly": "kesinlikle",
    "certificate": "sertifika",
    "chain": "Zincir",
    "chair": "sandalye",
    "chairman": "başkan",
    "chairwoman": "kadın başkan",
    "challenge": "meydan okuma",
    "chamber": "bölme",
    "chance": "şans",
    "change": "değişiklik",
    "change round": "değişim yuvarlak",
    "channel": "kanal",
    "chapter": "bölüm",
    "character": "karakter",
    "characteristic": "karakteristik",
    "charge": "şarj etmek",
    "charity": "sadaka",
    "chart": "grafik",
    "chase": "kovalamak",
    "chase away": "kovalamak",
    "chat": "sohbet",
    "cheap": "ucuz",
    "cheaply": "ucuza",
    "cheat": "hile",
    "cheat of": "arasında hile",
    "check": "Kontrol",
    "check in": "giriş",
    "check into": "içine çek",
    "check on": "kontrol et",
    "check out": "ödeme",
    "check over": "incelemek",
    "check up on": "kontrol etmek",
    "cheek": "yanak",
    "cheerful": "neşeli",
    "cheerfully": "neşeyle",
    "cheese": "peynir",
    "chemical": "kimyasal",
    "chemist": "eczacı",
    "chemistry": "kimya",
    "cheque": "Kontrol",
    "chest": "göğüs",
    "chew": "çiğnemek",
    "chicken": "tavuk",
    "chief": "şef",
    "child": "çocuk",
    "chin": "Çene",
    "chip": "yonga",
    "chocolate": "çikolata",
    "choice": "tercih",
    "choose": "Seç",
    "chop": "pirzola",
    "chop down": "devirmek",
    "chop off": "kesmek",
    "church": "kilise",
    "cigarette": "sigara",
    "cinema": "sinema",
    "circle": "daire",
    "circumstance": "durum",
    "citizen": "vatandaş",
    "city": "Kent",
    "civil": "sivil",
    "claim": "İddia",
    "clap": "alkış",
    "class": "sınıf",
    "classic": "klasik",
    "classroom": "sınıf",
    "clean": "temiz",
    "clean up": "Temizlemek",
    "clear": "açık",
    "clearly": "Açıkça",
    "clear out": "boşaltmak",
    "clear up": "temizlemek",
    "clerk": "kâtip",
    "clever": "zeki",
    "click": "Tıklayın",
    "client": "müşteri",
    "climate": "iklim",
    "climb": "tırmanış",
    "climbing": "Tırmanmak",
    "clock": "saat",
    "close 1": "yakın 1",
    "close 2": "yakın 2",
    "closed": "kapalı",
    "closely": "yakından",
    "closet": "dolap",
    "cloth": "bez",
    "clothes": "çamaşırlar",
    "clothing": "Giyim",
    "cloud": "bulut",
    "club": "kulüp",
    "coach": "Koç",
    "coal": "kömür",
    "coast": "sahil",
    "coat": "ceket",
    "code": "kod",
    "coffee": "Kahve",
    "coin": "madeni para",
    "cold": "soğuk",
    "coldly": "sakinlikle",
    "collapse": "çöküş",
    "colleague": "çalışma arkadaşı",
    "collect": "toplamak",
    "collection": "Toplamak",
    "college": "kolej",
    "colour": "renk",
    "coloured": "renkli",
    "column": "kolon",
    "combination": "kombinasyon",
    "combine": "birleştirmek",
    "come": "gel",
    "come across": "karşılaşmak",
    "come down": "sakinleş",
    "comedy": "komedi",
    "come from": "dan geliyorum",
    "come in": "Girin",
    "come off": "dökülmek",
    "come on": "Haydi",
    "come out": "çıkmak",
    "come round": "ayılmak",
    "come to": "gel",
    "come up": "ortaya çıkmak",
    "come up to": "gelmek",
    "comfort": "konfor",
    "comfortable": "rahat",
    "comfortably": "rahatça",
    "command": "komuta",
    "comment": "yorum Yap",
    "commercial": "ticari",
    "commission": "komisyon",
    "commit": "işlemek",
    "commitment": "taahhüt",
    "committee": "Kurul",
    "common": "Yaygın",
    "commonly": "çoğunlukla",
    "communicate": "iletişim kurmak",
    "communication": "iletişim",
    "community": "topluluk",
    "company": "şirket",
    "compare": "karşılaştırmak",
    "comparison": "karşılaştırma",
    "compete": "rekabet",
    "competition": "yarışma",
    "competitive": "rekabetçi",
    "complain": "şikayet",
    "complaint": "şikâyet",
    "complete": "tamamlayınız",
    "completely": "tamamen",
    "complex": "karmaşık",
    "complicate": "komplike",
    "complicated": "karmaşık",
    "computer": "bilgisayar",
    "concentrate": "yoğunlaşmak",
    "concentrate on": "konsantre olmak",
    "concentration": "konsantrasyon",
    "concept": "kavram",
    "concern": "ilgilendirmek",
    "concerned": "endişeli",
    "concerning": "ilişkin",
    "concert": "konser",
    "conclude": "sonuçlandırmak",
    "conclusion": "Sonuç",
    "concrete": "Somut",
    "condition": "şart",
    "conduct": "yönetmek",
    "conference": "konferans",
    "confidence": "güven",
    "confident": "kendine güvenen",
    "confidently": "kendine güvenerek",
    "confine": "confine",
    "confined": "izole edilip saklanmış",
    "confirm": "onaylamak",
    "conflict": "fikir ayrılığı",
    "confront": "karşısına çıkmak",
    "confuse": "şaşırtmak",
    "confused": "Şaşkın",
    "confusing": "kafa karıştırıcı",
    "confusion": "bilinç bulanıklığı, konfüzyon",
    "congratulate": "tebrik",
    "congratulation": "tebrik",
    "congress": "kongre",
    "connect": "connect",
    "connected": "bağlı",
    "connection": "bağ",
    "conscious": "bilinçli",
    "consequence": "sonuç",
    "conservative": "muhafazakâr",
    "consider": "düşünmek",
    "considerable": "önemli",
    "considerably": "önemli ölçüde",
    "consideration": "düşünce",
    "consist": "oluşmaktadır",
    "consist of": "oluşmaktadır",
    "constant": "sabit",
    "constantly": "sürekli",
    "construct": "yapı",
    "construction": "inşaat",
    "consult": "danışmak",
    "consumer": "tüketici",
    "contact": "İletişim",
    "contain": "içermek",
    "container": "konteyner",
    "contemporary": "çağdaş",
    "content 1": "içerik 1",
    "contest": "yarışma",
    "context": "bağlam",
    "continent": "kıta",
    "continue": "devam et",
    "continuous": "sürekli",
    "continuously": "devamlı olarak",
    "contract": "sözleşme",
    "contrast": "kontrast",
    "contrasting": "zıt",
    "contribute": "katkıda bulunmak",
    "contribution": "katkı",
    "control": "kontrol",
    "controlled": "kontrollü",
    "convenient": "uygun",
    "convention": "ortak düşünce",
    "conventional": "Konvansiyonel",
    "conversation": "konuşma",
    "convert": "dönüştürmek",
    "convince": "ikna etmek",
    "cook": "pişirmek",
    "cooker": "Ocak",
    "cookie": "kurabiye",
    "cooking": "yemek pişirme",
    "cool": "güzel",
    "cool down": "sakin ol",
    "cope": "başa çıkmak",
    "copy": "kopya",
    "core": "çekirdek",
    "corner": "köşe",
    "correct": "true",
    "correctly": "doğru şekilde",
    "cost": "maliyet",
    "cottage": "kulübe",
    "cotton": "pamuk",
    "cough": "öksürük",
    "coughing": "öksürme",
    "could": "abilir",
    "council": "konsey",
    "count": "Miktar",
    "counter": "sayaç",
    "count on": "güvenmek",
    "country": "ülke",
    "countryside": "kırsal bölge",
    "county": "ilçe",
    "couple": "çift",
    "courage": "cesaret",
    "course": "kurs",
    "court": "mahkeme",
    "cousin": "hala kızı",
    "cover": "örtmek",
    "covered": "kapalı",
    "covering": "kaplama",
    "cover up": "Örtmek",
    "cow": "inek",
    "crack": "çatlak",
    "cracked": "Çatlak",
    "craft": "zanaat",
    "crash": "kaza",
    "crazy": "çılgın",
    "cream": "krem",
    "create": "oluşturmak",
    "creature": "yaratık",
    "credit": "kredi",
    "credit card": "kredi kartı",
    "crime": "suç",
    "criminal": "adli",
    "crisis": "kriz",
    "crisp": "gevrek",
    "criterion": "kriter",
    "critical": "kritik",
    "criticism": "eleştiri",
    "criticize": "eleştirmek",
    "crop": "ekin",
    "cross": "çapraz",
    "cross out": "çizmek",
    "crowd": "kalabalık",
    "crowded": "kalabalık",
    "crown": "taç",
    "crucial": "çok önemli",
    "cruel": "zalim",
    "crush": "ezme",
    "cry": "Ağla",
    "cry out": "haykırmak",
    "cultural": "kültürel",
    "culture": "kültür",
    "cup": "Fincan",
    "cupboard": "dolap",
    "curb": "zapt etmek",
    "cure": "Çare",
    "curious": "Meraklı",
    "curiously": "merakla",
    "curl": "bukle",
    "curl up": "kıvrılmak",
    "curly": "Kıvırcık",
    "current": "şimdiki",
    "currently": "şu anda",
    "curtain": "perde",
    "curve": "eğri",
    "curved": "kavisli",
    "custom": "görenek",
    "customer": "müşteri",
    "customs": "Gümrük",
    "cut": "kesmek",
    "cut back": "azaltmak",
    "cut down": "kısmak",
    "cut off": "ayırmak",
    "cut out": "kesmek",
    "cut up": "dilimlemek",
    "cycle": "döngü",
    "cycling": "bisiklet sürmek",
    "dad": "baba",
    "daily": "günlük",
    "damage": "hasar",
    "damp": "nemli",
    "dance": "dans",
    "dancer": "dansçı",
    "dancing": "dans",
    "danger": "Tehlike",
    "dangerous": "tehlikeli",
    "dare": "cesaret etmek",
    "dark": "karanlık",
    "data": "veri",
    "date": "tarih",
    "date back": "tarih geri",
    "daughter": "kız evlat",
    "day": "gün",
    "dead": "ölü",
    "deaf": "sağır",
    "deal": "anlaştık mı",
    "deal in": "anlaşma",
    "deal with": "uğraşmak",
    "dear": "Sayın",
    "death": "ölüm",
    "debate": "tartışma",
    "debt": "borç",
    "decade": "onyıl",
    "decay": "çürüme",
    "December": "Aralık",
    "decide": "karar ver",
    "decide on": "karar verilen",
    "decision": "karar",
    "declare": "bildirmek",
    "decline": "düşüş",
    "decorate": "süslemek",
    "decoration": "dekorasyon",
    "decorative": "dekoratif",
    "decrease": "azaltmak",
    "deep": "derin",
    "deeply": "derinden",
    "defeat": "yenilgi",
    "defence": "savunma",
    "defend": "savunmak",
    "define": "tanımlamak",
    "definite": "kesin",
    "definitely": "kesinlikle",
    "definition": "tanım",
    "degree": "derece",
    "delay": "gecikme",
    "deliberate": "kasten, kasıtlı, planlı",
    "deliberately": "kasten",
    "delicate": "narin",
    "delight": "lokum",
    "delighted": "memnun olmak",
    "deliver": "teslim etmek",
    "delivery": "teslim",
    "demand": "talep",
    "demonstrate": "göstermek",
    "dentist": "diş doktoru",
    "deny": "reddetmek",
    "department": "Bölüm",
    "departure": "kalkış",
    "depend": "bağımlı",
    "depend on": "bağlıdır",
    "deposit": "Depozito",
    "depress": "bunalıma girmek",
    "depressed": "bunalımlı",
    "depressing": "iç karartıcı",
    "depth": "derinlik",
    "derive": "derived",
    "derive from": "türetmek",
    "describe": "tanımlamak",
    "description": "açıklama",
    "desert": "çöl",
    "deserted": "ıssız",
    "deserve": "hak etmek",
    "design": "dizayn",
    "desire": "arzu etmek",
    "desk": "sıra",
    "desperate": "umutsuz",
    "desperately": "umutsuzca",
    "despite": "rağmen",
    "destroy": "yıkmak",
    "destruction": "imha",
    "detail": "detay",
    "detailed": "detaylı",
    "determination": "kararlılık",
    "determine": "belirlemek",
    "determined": "belirlenen",
    "develop": "geliştirmek",
    "development": "gelişme",
    "device": "cihaz",
    "devote": "adamak",
    "devoted": "sadık",
    "devote to": "kendini bir şeye adamak",
    "diagram": "diyagram",
    "diamond": "elmas",
    "diary": "günlük",
    "dictionary": "sözlük",
    "die": "ölmek",
    "die away": "kesilmek",
    "die out": "nesli tükenmek",
    "diet": "diyet",
    "difference": "fark",
    "different": "farklı",
    "differently": "farklı olarak",
    "difficult": "zor",
    "difficulty": "zorluk",
    "dig": "kazmak",
    "digital": "dijital",
    "dinner": "akşam yemegi",
    "direct": "direkt",
    "direction": "yön",
    "directly": "direkt olarak",
    "director": "yönetmen",
    "dirt": "kir",
    "dirty": "kirli",
    "disabled": "engelli",
    "disadvantage": "dezavantaj",
    "disagree": "katılmıyorum",
    "disagreement": "anlaşmazlık",
    "disagree with doing": "yapıyor katılmıyorum",
    "disappear": "kaybolmak",
    "disappoint": "hayal kırıklığına uğratmak",
    "disappointed": "hüsrana uğramış",
    "disappointing": "hayal kırıklığı",
    "disappointment": "hayal kırıklığı",
    "disapproval": "onaylamama",
    "disapprove": "disapprove",
    "disapproving": "onaylamayan",
    "disaster": "afet",
    "disc": "disk",
    "discipline": "disiplin",
    "discount": "indirim",
    "discover": "discover",
    "discovery": "keşif",
    "discuss": "tartışmak",
    "discussion": "tartışma",
    "disease": "hastalık",
    "disgust": "iğrenme",
    "disgusted": "iğrenmiş",
    "disgusting": "iğrenç",
    "dish": "tabak",
    "dishonest": "sahtekâr",
    "dishonestly": "namussuzca",
    "disk": "disk",
    "dislike": "Beğenmemek",
    "dismiss": "Reddet",
    "display": "Görüntüle",
    "dissolve": "eritmek",
    "distance": "mesafe",
    "distinguish": "ayırmak",
    "distribute": "dağıtmak",
    "distribution": "dağıtım",
    "district": "ilçe",
    "disturb": "bozmak",
    "disturbing": "rahatsız edici",
    "divide": "bölmek",
    "division": "bölünme",
    "divorce": "boşanma",
    "divorced": "boşanmış",
    "do 1": "1 do",
    "doctor": "doktor",
    "document": "belge",
    "dog": "köpek",
    "dollar": "dolar",
    "domestic": "yerli",
    "dominate": "baskın",
    "door": "kapı",
    "dot": "nokta",
    "double": "çift",
    "doubt": "şüphe",
    "do up": "paketlemek",
    "do with": "ile yapmak",
    "do without": "sız yapmak",
    "down": "aşağı",
    "downstairs": "alt kat",
    "downward": "aşağıya doğru",
    "downwards": "aşağıya doğru",
    "dozen": "düzine",
    "draft": "taslak",
    "drag": "sürüklemek",
    "drama": "dram",
    "dramatic": "dramatik",
    "dramatically": "dramatik",
    "draw": "çizmek",
    "drawer": "Çekmece",
    "drawing": "çizim",
    "dream": "rüya",
    "dress": "elbise",
    "dressed": "giyinmiş",
    "dress up": "süslemek",
    "drink": "İçmek",
    "drive": "sürücü",
    "drive away": "arabayla uzaklaşmak",
    "drive off": "Gitmek",
    "driver": "sürücü",
    "driving": "sürme",
    "drop": "düşürmek",
    "drop out": "bırakmak",
    "drug": "uyuşturucu madde",
    "drugstore": "eczane",
    "drum": "tambur",
    "drunk": "sarhoş",
    "dry": "kuru",
    "dry off": "kapalı kuru",
    "dry up": "kurutmak",
    "due": "nedeniyle",
    "dull": "donuk",
    "dump": "çöplük",
    "during": "sırasında",
    "dust": "toz",
    "duty": "görev",
    "DVD": "DVD",
    "dying": "ölen",
    "each": "her biri",
    "each other": "herbiri",
    "ear": "kulak",
    "early": "erken",
    "earn": "kazanmak",
    "earth": "Dünya",
    "ease": "kolaylaştırmak",
    "easily": "kolayca",
    "east": "Doğu",
    "eastern": "doğu",
    "easy": "kolay",
    "eat": "yemek",
    "eat out": "dışarıda yemek",
    "eat up": "yiyip bitirmek",
    "economic": "ekonomik",
    "economy": "ekonomi",
    "edge": "kenar",
    "edition": "baskı",
    "editor": "editör",
    "educate": "Eğitim vermek",
    "educated": "eğitimli",
    "education": "Eğitim",
    "effect": "etki",
    "effective": "etkili",
    "effectively": "etkili bir şekilde",
    "efficient": "verimli",
    "efficiently": "verimli biçimde",
    "effort": "çaba",
    "e.g.": "Örneğin.",
    "egg": "Yumurta",
    "eight": "sekiz",
    "eighteen": "onsekiz",
    "eighteenth": "onsekizinci",
    "eighth": "sekizinci",
    "eightieth": "sekseninci",
    "eighty": "seksen",
    "either": "ya",
    "elbow": "dirsek",
    "elderly": "yaşlı",
    "elect": "seçilmiş",
    "election": "seçim",
    "electric": "elektrik",
    "electrical": "elektriksel",
    "electricity": "elektrik",
    "electronic": "elektronik",
    "elegant": "zarif",
    "element": "eleman",
    "elevator": "asansör",
    "eleven": "on bir",
    "eleventh": "onbirinci",
    "else": "Başka",
    "elsewhere": "başka yerde",
    "email": "E-posta",
    "embarrass": "utandırmak",
    "embarrassed": "mahçup",
    "embarrassing": "utandırıcı",
    "embarrassment": "sıkıntı",
    "emerge": "ortaya çıkmak",
    "emergency": "acil Durum",
    "emotion": "duygu",
    "emotional": "duygusal",
    "emotionally": "duygusal yönden",
    "emphasis": "vurgu",
    "emphasize": "vurgulamak",
    "empire": "imparatorluk",
    "employ": "kullanmak",
    "employee": "işçi",
    "employer": "işveren",
    "employment": "iş",
    "empty": "boş",
    "enable": "etkinleştirme",
    "encounter": "karşılaşma",
    "encourage": "teşvik etmek",
    "encouragement": "teşvik",
    "end": "son",
    "end in": "sona",
    "ending": "bitirme",
    "end up": "sonuçlanmak",
    "enemy": "düşman",
    "energy": "enerji",
    "engage": "Tut",
    "engaged": "nişanlı",
    "engine": "motor",
    "engineer": "mühendis",
    "engineering": "mühendislik",
    "enjoy": "zevk almak",
    "enjoyable": "zevkli",
    "enjoyment": "hoşlanma",
    "enormous": "muazzam",
    "enough": "yeter",
    "enquiry": "soruşturma",
    "ensure": "sağlamak",
    "enter": "giriş",
    "entertain": "eğlendirmek",
    "entertainer": "şovmen",
    "entertaining": "eğlenceli",
    "entertainment": "eğlence",
    "enthusiasm": "coşku",
    "enthusiastic": "hevesli",
    "enthusiastically": "şevkle",
    "entire": "tüm",
    "entirely": "Baştan sona",
    "entitle": "baslığı",
    "entrance 1": "giriş 1",
    "entry": "giriş",
    "envelope": "zarf",
    "environment": "çevre",
    "environmental": "çevre",
    "equal": "eşit",
    "equally": "aynı derecede",
    "equipment": "ekipman",
    "equivalent": "eşdeğer",
    "error": "hata",
    "escape": "kaçış",
    "especially": "özellikle",
    "essay": "makale",
    "essential": "gerekli",
    "essentially": "esasen",
    "establish": "kurmak",
    "estate": "arazi",
    "estimate": "tahmin",
    "etc.": "vb.",
    "euro": "euro",
    "even": "hatta",
    "evening": "akşam",
    "event": "Etkinlik",
    "eventually": "Sonuçta",
    "ever": "hiç",
    "every": "her",
    "everybody": "herkes",
    "everyone": "herkes",
    "everything": "her şey",
    "everywhere": "her yerde",
    "evidence": "kanıt",
    "evil": "kötülük",
    "ex-": "eski",
    "exact": "kesin",
    "exactly": "kesinlikle",
    "exaggerate": "fahiş",
    "exaggerated": "abartılı",
    "exam": "sınav",
    "examination": "sınav",
    "examine": "muayene etmek",
    "example": "misal",
    "excellent": "mükemmel",
    "except": "dışında",
    "exception": "istisna",
    "exchange": "değiş tokuş",
    "excite": "heyecanlandırmak",
    "excited": "uyarılmış",
    "excitement": "heyecan",
    "exciting": "heyecan verici",
    "exclude": "dışlamak",
    "excluding": "hariç",
    "excuse": "bahane",
    "executive": "yönetici",
    "exercise": "egzersiz yapmak",
    "exhibit": "sergi",
    "exhibition": "sergi",
    "exist": "var olmak",
    "existence": "varoluş",
    "exit": "çıkış",
    "expand": "genişletmek",
    "expect": "beklemek",
    "expectation": "beklenti",
    "expected": "beklenen",
    "expense": "gider",
    "expensive": "pahalı",
    "experience": "deneyim",
    "experienced": "Tecrübeli",
    "experiment": "Deney",
    "expert": "uzman",
    "explain": "açıklamak",
    "explanation": "açıklama",
    "explode": "patlamak",
    "explore": "keşfetmek",
    "explosion": "patlama",
    "export": "ihracat",
    "expose": "maruz bırakmak",
    "express": "ekspres",
    "expression": "ifade",
    "extend": "uzatmak",
    "extension": "uzantı",
    "extensive": "geniş",
    "extent": "derece",
    "extra": "ekstra",
    "extraordinary": "olağanüstü",
    "extreme": "aşırı",
    "extremely": "son derece",
    "eye": "göz",
    "face": "yüz",
    "face up to": "yüzleşmek",
    "facility": "tesis",
    "fact": "gerçek",
    "factor": "faktör",
    "factory": "fabrika",
    "fail": "başarısız",
    "failure": "başarısızlık",
    "faint": "baygın",
    "faintly": "hafiften",
    "fair": "fuar",
    "fairly": "oldukça",
    "faith": "inanç",
    "faithful": "sadık",
    "faithfully": "dürüstçe",
    "fall": "sonbahar",
    "false": "yanlış",
    "fame": "şöhret",
    "familiar": "tanıdık",
    "family": "aile",
    "famous": "tanınmış",
    "fan": "yelpaze",
    "fancy": "fantezi",
    "far": "Irak",
    "farm": "Çiftlik",
    "farmer": "çiftçi",
    "farming": "tarım",
    "farther": "daha uzağa",
    "farthest": "en uzak",
    "fashion": "moda",
    "fashionable": "moda",
    "fast": "hızlı",
    "fasten": "bağlamak",
    "fat": "şişman",
    "father": "baba",
    "faucet": "musluk",
    "fault": "hata",
    "favour": "iyilik",
    "favourite": "favori",
    "fear": "korku",
    "feather": "tüy",
    "feature": "özellik",
    "February": "Şubat",
    "federal": "federal",
    "fee": "ücret",
    "feed": "besleme",
    "feel": "hissetmek",
    "feeling": "duygu",
    "fellow": "dost",
    "female": "kadın",
    "fence": "çit",
    "festival": "Festival",
    "fetch": "getirmek",
    "fever": "ateş",
    "few": "az",
    "field": "alan",
    "fifteen": "onbeş",
    "fifteenth": "onbeşinci",
    "fifth": "beşinci",
    "fiftieth": "ellinci",
    "fifty": "elli",
    "fight": "kavga",
    "fighting": "savaş",
    "figure": "şekil",
    "figure out": "çözmek",
    "file": "dosya",
    "fill": "dolgu",
    "fill in": "doldurun",
    "fill out": "doldur",
    "fill up": "doldur",
    "film": "film",
    "final": "nihai",
    "finally": "en sonunda",
    "finance": "maliye",
    "financial": "parasal",
    "find": "bulmak",
    "find out": "Bulmak",
    "fine": "ince",
    "finely": "incecik",
    "finger": "parmak",
    "finish": "bitiş",
    "finished": "bitmiş",
    "finish off": "bitirmek",
    "fire": "ateş",
    "firm": "firma",
    "firmly": "sıkıca",
    "first": "ilk",
    "fish": "balık",
    "fishing": "Balık tutma",
    "fit": "Uygun",
    "fit in": "Uyum sağlamak",
    "five": "beş",
    "fix": "düzeltme",
    "fixed": "sabit",
    "flag": "bayrak",
    "flame": "alev",
    "flash": "flaş",
    "flat": "düz",
    "flavour": "lezzet",
    "flesh": "eti",
    "flight": "uçuş",
    "float": "şamandıra",
    "flood": "sel",
    "flooded": "sular altında",
    "flooding": "su baskını",
    "floor": "zemin",
    "flour": "un",
    "flow": "akış",
    "flower": "çiçek",
    "flu": "grip",
    "fly": "uçmak",
    "flying": "uçan",
    "focus": "odak",
    "fold": "kat",
    "folding": "katlama",
    "follow": "takip et",
    "following": "takip etme",
    "follow up": "takip et",
    "food": "Gıda",
    "foot": "ayak",
    "football": "Futbol",
    "for": "için",
    "force": "güç",
    "forecast": "tahmin",
    "foreign": "Dış",
    "forest": "orman",
    "forever": "sonsuza dek",
    "forget": "unutmak",
    "forgive": "affetmek",
    "fork": "çatal",
    "form": "form",
    "formal": "biçimsel",
    "formally": "resmen",
    "former": "eski",
    "formerly": "vakti zamanında",
    "formula": "formül",
    "fortieth": "kırkıncı",
    "fortune": "servet",
    "forty": "kırk",
    "forward": "ileri",
    "found": "bulundu",
    "foundation": "Yapı temeli",
    "four": "dört",
    "fourteen": "on dört",
    "fourteenth": "on dördüncü",
    "fourth": "dördüncü",
    "frame": "çerçeve",
    "free": "Bedava",
    "freedom": "özgürlük",
    "freely": "serbestçe",
    "freeze": "donmak",
    "frequent": "sık",
    "frequently": "sık sık",
    "fresh": "taze",
    "freshly": "taze",
    "Friday": "Cuma",
    "fridge": "buzdolabı",
    "friend": "arkadaş",
    "friendly": "arkadaş canlısı",
    "friendship": "dostluk",
    "frighten": "korkutmak",
    "frighten away/off": "korkutup / kapalı",
    "frightened": "korkmuş",
    "frightening": "korkutucu",
    "from": "itibaren",
    "front": "ön",
    "frozen": "dondurulmuş",
    "fruit": "meyve",
    "fry": "kızartmak",
    "fuel": "yakıt",
    "full": "tam",
    "fully": "tamamen",
    "fun": "eğlence",
    "function": "fonksiyon",
    "function as": "işlev olarak",
    "fund": "fon, sermaye",
    "fundamental": "temel",
    "funeral": "cenaze",
    "funny": "komik",
    "fur": "kürk",
    "furniture": "mobilya",
    "further": "Daha ileri",
    "future": "gelecek",
    "gain": "kazanç",
    "gallon": "galon",
    "gamble": "kumar",
    "gambling": "kumar",
    "game": "oyun",
    "gap": "boşluk",
    "garage": "garaj",
    "garbage": "çöp",
    "garden": "Bahçe",
    "gas": "gaz",
    "gasoline": "benzin",
    "gate": "kapı",
    "gather": "toplamak",
    "gear": "vites",
    "general": "genel",
    "generally": "genellikle",
    "generate": "üretmek",
    "generation": "nesil",
    "generous": "cömert",
    "generously": "Cömertçe",
    "gentle": "nazik",
    "gentleman": "beyefendi",
    "gently": "nazikçe",
    "genuine": "hakiki",
    "genuinely": "gerçekten",
    "geography": "coğrafya",
    "get": "almak",
    "get away": "kurtulmak",
    "get away with": "paçayı",
    "get back": "geri gel",
    "get by": "idare etmek",
    "get in": "Alın",
    "get into": "içine girmek",
    "get off": "inmek",
    "get on": "bin",
    "get on with": "ile almak",
    "get out of": "kurtulmak",
    "get over": "atlatmak",
    "get round": "yayılmak",
    "get round to": "yuvarlak olsun",
    "get through": "Üstesinden gelmek",
    "get up": "kalkmak",
    "giant": "dev",
    "gift": "hediye",
    "girl": "kız",
    "girlfriend": "kız arkadaşı",
    "give": "vermek",
    "give away": "çekiliş",
    "give back": "geri ver",
    "give in": "vermek",
    "give off": "yaymak",
    "give out": "yaymak",
    "give up": "vazgeçmek",
    "glad": "memnun",
    "glass": "bardak",
    "global": "global",
    "glove": "eldiven",
    "glue": "tutkal",
    "go": "Git",
    "go ahead": "devam et",
    "goal": "hedef",
    "go away": "çekip gitmek",
    "go back": "geri gitmek",
    "go back to": "geri dönmek",
    "go by": "gitmek",
    "god": "Tanrı",
    "go down": "aşağı in",
    "go into": "girmek",
    "gold": "altın",
    "good": "iyi",
    "goodbye": "Güle güle",
    "goods": "mal",
    "go off": "çıkmak",
    "go on": "devam et",
    "go on doing": "yapmaya devam",
    "go out": "Git dışarı",
    "go out with": "ile çıkmak",
    "go over": "üzerinden gitmek",
    "go round": "kurcalamak",
    "go through": "geçmesi",
    "go through with": "yürütmek",
    "go to": "gidin",
    "go up": "yukarı git",
    "govern": "yönetmek",
    "government": "hükümet",
    "governor": "Vali",
    "go with": "ile git",
    "go without": "onsuz git",
    "grab": "kapmak",
    "grade": "sınıf",
    "gradual": "kademeli",
    "gradually": "kademeli olarak",
    "grain": "tane",
    "gram": "gram",
    "grammar": "dilbilgisi",
    "grand": "ulu",
    "grandchild": "torun",
    "granddaughter": "kız torun",
    "grandfather": "Büyük baba",
    "grandmother": "Nene",
    "grandparent": "büyükbaba veya büyükanne",
    "grandson": "erkek torun",
    "grant": "hibe",
    "grass": "çimen",
    "grateful": "minnettar",
    "grave 1": "mezar 1",
    "gravely": "ciddi",
    "great": "harika",
    "greatly": "çokça",
    "green": "yeşil",
    "grey": "gri",
    "grocery": "Bakkal",
    "ground": "zemin",
    "group": "grup",
    "grow": "büyümek",
    "growth": "büyüme",
    "grow up": "büyümek",
    "guarantee": "garanti",
    "guard": "bekçi",
    "guess": "tahmin",
    "guest": "misafir",
    "guide": "kılavuz",
    "guilty": "suçlu",
    "gun": "tabanca",
    "guy": "insan",
    "habit": "alışkanlık",
    "hair": "saç",
    "hairdresser": "kuaför",
    "half": "yarım",
    "hall": "salon",
    "hammer": "çekiç",
    "hand": "el",
    "hand back": "geri vermek",
    "hand down": "kararı bildirmek",
    "hand in": "El içinde",
    "handle": "üstesinden gelmek",
    "hand out": "dağıtmak",
    "hand over": "teslim et",
    "hand round": "El yuvarlak",
    "hang": "asmak",
    "hang about": "asmak hakkında",
    "hang about with": "asmak ile ilgili",
    "hang around": "takılmak",
    "hang around with": "takılmak",
    "hang on": "Dayan",
    "hang on to": "asmak",
    "hang up": "telefonu kapatmak",
    "happen": "olmak",
    "happen to": "başına",
    "happily": "mutlulukla",
    "happiness": "mutluluk",
    "happy": "mutlu",
    "hard": "zor",
    "hardly": "zorlukla",
    "harm": "zarar",
    "harmful": "zararlı",
    "harmless": "zararsız",
    "hat": "şapka",
    "hate": "nefret",
    "hatred": "nefret",
    "have": "Sahip olmak",
    "have back": "geri alabilir",
    "have on": "sahip",
    "have to": "zorunda",
    "he": "o",
    "head": "kafa",
    "headache": "baş ağrısı",
    "heal": "iyileşmek",
    "health": "sağlık",
    "healthy": "sağlıklı",
    "hear": "duymak",
    "hear from": "dan duymak",
    "hearing": "işitme",
    "hear of": "duymak",
    "heart": "kalp",
    "heat": "sıcaklık",
    "heating": "ısıtma",
    "heat up": "ısıtmak",
    "heaven": "cennet",
    "heavily": "ağır şekilde",
    "heavy": "ağır",
    "heel": "topuk",
    "height": "yükseklik",
    "hell": "cehennem",
    "hello": "Merhaba",
    "help": "Yardım",
    "helpful": "faydalı",
    "help out": "yardım",
    "hence": "bundan dolayı",
    "her": "ona",
    "here": "İşte",
    "hero": "kahraman",
    "hers": "onunki",
    "herself": "kendini",
    "hesitate": "tereddüt",
    "hi": "Selam",
    "hide": "saklamak",
    "high": "yüksek",
    "highlight": "vurgulamak",
    "highly": "büyük ölçüde",
    "highway": "karayolu",
    "hill": "Tepe",
    "him": "onu",
    "himself": "kendisi",
    "hip": "kalça",
    "hire": "kiralama",
    "hire out": "kiraya vermek",
    "his": "onun",
    "historical": "tarihi",
    "history": "Tarih",
    "hit": "isabet",
    "hobby": "hobi",
    "hold": "ambar",
    "hold back": "tutma geri",
    "hold on": "tut",
    "hold on to": "tutunmaya",
    "hold out": "uzatmak",
    "hold up": "gecikmek",
    "hole": "delik",
    "holiday": "tatil",
    "hollow": "oyuk",
    "holy": "kutsal",
    "home": "ev",
    "homework": "ödev",
    "honest": "dürüst",
    "honestly": "dürüstçe",
    "honour": "Onur",
    "hook": "kanca",
    "hope": "umut",
    "horizontal": "yatay",
    "horn": "Boynuz",
    "horror": "korku",
    "horse": "at",
    "hospital": "hastane",
    "host": "evsahibi",
    "hot": "Sıcak",
    "hotel": "otel",
    "hour": "saat",
    "house": "ev",
    "household": "ev halkı",
    "housing": "Konut",
    "how": "Nasıl",
    "however": "ancak",
    "huge": "Kocaman",
    "human": "insan",
    "humorous": "nükteli, komik",
    "humour": "Mizah",
    "hundred": "yüz",
    "hundredth": "yüzüncü",
    "hungry": "aç",
    "hunt": "avlanmak",
    "hunting": "avcılık",
    "hurry": "acele",
    "hurry up": "acele et",
    "hurt": "canını yakmak",
    "husband": "koca",
    "I": "ben",
    "ice": "buz",
    "ice cream": "dondurma",
    "idea": "Fikir",
    "ideal": "ideal",
    "identify": "belirlemek",
    "identify with": "ile tespit",
    "identity": "Kimlik",
    "i.e.": "diğer bir deyişle",
    "if": "Eğer",
    "ignore": "göz ardı etmek",
    "ill": "hasta",
    "illegal": "yasadışı",
    "illegally": "yasadışı",
    "illness": "hastalık",
    "illustrate": "gözünde canlandırmak",
    "image": "görüntü",
    "imaginary": "hayali",
    "imagination": "hayal gücü",
    "imagine": "hayal etmek",
    "immediate": "acil",
    "immediately": "hemen",
    "immoral": "ahlaksız",
    "impact": "etki",
    "impatient": "sabırsız",
    "implication": "Ima",
    "imply": "ima etmek",
    "import": "ithalat",
    "importance": "önem",
    "important": "önemli",
    "importantly": "önemlisi",
    "impose": "empoze etmek",
    "impossible": "imkansız",
    "impress": "etkilemek",
    "impressed": "etkilendim",
    "impression": "izlenim",
    "impressive": "etkileyici",
    "improve": "iyileştirmek",
    "improvement": "Gelişme",
    "in": "içinde",
    "inability": "yeteneksizlik",
    "inch": "inç",
    "incident": "olay",
    "include": "Dahil etmek",
    "including": "dahil olmak üzere",
    "income": "Gelir",
    "increase": "artırmak",
    "increasingly": "giderek",
    "indeed": "aslında",
    "independence": "bağımsızlık",
    "independent": "bağımsız",
    "independently": "bağımsız",
    "index": "indeks",
    "indicate": "belirtmek",
    "indication": "belirti",
    "indirect": "dolaylı",
    "indirectly": "dolaylı olarak",
    "individual": "bireysel",
    "indoor": "kapalı",
    "indoors": "içeriye",
    "industrial": "Sanayi",
    "industry": "sanayi",
    "inevitable": "kaçınılmaz",
    "inevitably": "kaçınılmaz",
    "infect": "bulaştırmak",
    "infected": "enfekte",
    "infection": "enfeksiyon",
    "infectious": "bulaşıcı",
    "influence": "etkilemek",
    "inform": "bilgi vermek",
    "informal": "resmi olmayan",
    "information": "bilgi",
    "ingredient": "bileşen",
    "initial": "ilk",
    "initially": "başlangıçta",
    "initiative": "girişim",
    "injure": "yaralamıyorum",
    "injured": "yaralı",
    "injury": "yaralanma",
    "ink": "mürekkep",
    "inner": "iç",
    "innocent": "masum",
    "insect": "böcek",
    "insert": "insert",
    "inside": "içeride",
    "insist": "ısrar etmek",
    "insist on": "ısrar",
    "insist on doing": "yapmakta ısrar",
    "install": "Yüklemek",
    "instance": "örnek",
    "instead": "yerine",
    "instead of": "onun yerine",
    "institute": "enstitü",
    "institution": "kurum",
    "instruction": "talimat",
    "instrument": "müzik aleti",
    "insult": "hakaret",
    "insulting": "hakaret",
    "insurance": "sigorta",
    "intelligence": "zeka",
    "intelligent": "akıllı",
    "intend": "niyet etmek",
    "intended": "istenilen",
    "intention": "niyet",
    "interest": "faiz",
    "interested": "Ilgilenen",
    "interesting": "ilginç",
    "interior": "iç",
    "internal": "iç",
    "international": "Uluslararası",
    "Internet": "Internet",
    "interpret": "yorumlamak",
    "interpretation": "yorumlama",
    "interrupt": "kesmek",
    "interruption": "kesinti",
    "interval": "Aralık",
    "interview": "röportaj",
    "into": "içine",
    "introduce": "takdim etmek",
    "introduction": "Giriş",
    "invent": "icat etmek",
    "invention": "icat",
    "invest": "Invest",
    "investigate": "incelemek",
    "investigation": "soruşturma",
    "investment": "yatırım",
    "invitation": "davetiye",
    "invite": "Davet et",
    "involve": "dahil",
    "involved": "yer",
    "involvement": "ilgi",
    "iron": "Demir",
    "irritate": "kızdırmak",
    "irritated": "RAHATSIZ olmak",
    "irritating": "rahatsız edici",
    "island": "ada",
    "issue": "sorun",
    "it": "o",
    "item": "madde",
    "its": "onun",
    "itself": "kendisi",
    "jacket": "ceket",
    "jam": "reçel",
    "January": "Ocak",
    "jealous": "kıskanç",
    "jeans": "kot",
    "jelly": "jöle",
    "jewellery": "mücevher",
    "job": "iş",
    "join": "katılmak",
    "join in": "katıl",
    "joint": "bağlantı",
    "jointly": "ortaklaşa",
    "joke": "şaka",
    "journalist": "gazeteci",
    "journey": "seyahat",
    "joy": "sevinç",
    "judge": "hakim",
    "judgement": "yargı",
    "juice": "Meyve suyu",
    "July": "Temmuz",
    "jump": "atlama",
    "June": "Haziran",
    "junior": "genç",
    "just": "sadece",
    "justice": "adalet",
    "justified": "haklı",
    "justify": "haklı çıkarmak",
    "keen": "düşkün",
    "keep": "Tut",
    "keep out": "dışarıda tutmak",
    "keep out of": "uzak durmak",
    "keep up": "sürdür",
    "keep up with": "ayak uydurmak",
    "key": "anahtar",
    "keyboard": "tuş takımı",
    "kick": "Atmak",
    "kid": "çocuk",
    "kill": "öldürmek",
    "killing": "öldürme",
    "kilogram": "kilogram",
    "kilometre": "kilometre",
    "kind": "tür",
    "kindly": "nazikçe",
    "kindness": "iyilik",
    "king": "kral",
    "kiss": "öpücük",
    "kitchen": "mutfak",
    "knee": "diz",
    "knife": "bıçak",
    "knit": "örgü örmek",
    "knitted": "örgü",
    "knitting": "örme",
    "knock": "vurmak",
    "knock down": "yere sermek",
    "knock out": "Nakavt",
    "knot": "düğüm",
    "know": "bilmek",
    "knowledge": "bilgi",
    "lab": "laboratuvar",
    "label": "etiket",
    "laboratory": "laboratuvar",
    "labour": "emek",
    "lack": "eksiklik",
    "lacking": "eksik",
    "lady": "Hanım",
    "lake": "göl",
    "lamp": "Lamba",
    "land": "arazi",
    "landscape": "peyzaj",
    "lane": "Şerit",
    "language": "dil",
    "large": "büyük",
    "largely": "büyük oranda",
    "last 1": "son 1",
    "late": "geç",
    "later": "sonra",
    "latest": "En son",
    "latter": "ikincisi",
    "laugh": "gülmek",
    "laugh at": "e gülmek",
    "launch": "başlatmak",
    "law": "yasa",
    "lawyer": "avukat",
    "lay": "lay",
    "layer": "katman",
    "lazy": "tembel",
    "lead 1": "1 kurşun",
    "leader": "Önder",
    "leading 1": "1 lider",
    "leaf": "Yaprak",
    "league": "lig",
    "lean": "yağsız - Yağsız",
    "learn": "öğrenmek",
    "least": "en az",
    "leather": "deri",
    "leave": "ayrılmak",
    "leave out": "dışarıda bırakın",
    "lecture": "ders",
    "left": "ayrıldı",
    "leg": "bacak",
    "legal": "yasal",
    "legally": "yasal olarak",
    "lemon": "Limon",
    "lend": "ödünç vermek",
    "length": "uzunluk",
    "less": "Daha az",
    "lesson": "ders",
    "let": "İzin Vermek",
    "let down": "hayal ırıklığına uğratmak",
    "let off": "salmak",
    "letter": "mektup",
    "level": "seviye",
    "library": "kütüphane",
    "licence": "lisans",
    "license": "lisans",
    "lid": "kapak",
    "lie 1": "yalan 1",
    "lie 2": "yalan 2",
    "lie around": "etrafında yalan",
    "lie down": "yatmak",
    "life": "hayat",
    "lift": "asansör",
    "light": "ışık",
    "lightly": "hafifçe",
    "like": "sevmek",
    "likely": "muhtemelen",
    "limit": "sınır",
    "limited": "sınırlı",
    "limit to": "için sınır",
    "line": "hat",
    "link": "bağlantı",
    "lip": "dudak",
    "liquid": "sıvı",
    "list": "liste",
    "listen": "dinlemek",
    "literature": "Edebiyat",
    "litre": "litre",
    "little": "küçük",
    "live 1": "canlı 1",
    "live 2": "canlı 2",
    "lively": "canlı",
    "live on": "canlı üzerinde",
    "live through": "canlı aracılığıyla",
    "live together": "birlikte yaşamak",
    "living": "yaşam",
    "load": "yük",
    "loan": "borç",
    "local": "yerel",
    "locally": "lokal olarak",
    "locate": "yerleştirmek",
    "located": "bulunan",
    "location": "yer",
    "lock": "kilit",
    "lock up": "kilitlemek",
    "logic": "mantık",
    "logical": "mantıklı",
    "lonely": "yalnız",
    "long": "uzun",
    "look": "bak",
    "look after": "ilgilenmek",
    "look at": "bakmak",
    "look down on": "tepeden bakmak",
    "look forward to": "dört gözle beklemek",
    "look into": "içine bakmak",
    "look on": "bakmak",
    "look on with": "ile bakmak",
    "look out": "bak",
    "look out for": "dikkat etmek",
    "look round": "etrafa bak",
    "look through": "bakmak",
    "look up": "yukarı Bak",
    "look up to": "kadar bakmak",
    "loose": "gevşek",
    "loosely": "gevşek",
    "lord": "Kral",
    "lorry": "kamyon",
    "lose": "kaybetmek",
    "loss": "kayıp",
    "lost": "kayıp",
    "lot": "çok",
    "loud": "yüksek sesle",
    "loudly": "yüksek sesle",
    "love": "Aşk",
    "lovely": "güzel",
    "lover": "sevgili",
    "low": "düşük",
    "loyal": "sadık",
    "luck": "şans",
    "lucky": "şanslı",
    "luggage": "valiz",
    "lump": "yumru",
    "lunch": "öğle yemeği",
    "lung": "akciğer",
    "machine": "makine",
    "machinery": "makinalar",
    "mad": "deli",
    "magazine": "dergi",
    "magic": "sihirli",
    "mail": "posta",
    "main": "ana",
    "mainly": "esasen",
    "maintain": "sürdürmek",
    "major": "majör",
    "majority": "çoğunluk",
    "make": "Yapmak",
    "make into": "haline getirmek",
    "make up": "makyaj",
    "make-up": "makyaj",
    "make up for": "telafi",
    "male": "erkek",
    "mall": "alışveriş Merkezi",
    "man": "adam",
    "manage": "yönetmek",
    "management": "yönetim",
    "manager": "yönetici",
    "manner": "tavır",
    "manufacture": "üretim",
    "manufacturer": "üretici firma",
    "manufacturing": "imalat",
    "many": "çok",
    "map": "harita",
    "march": "Mart",
    "March": "Mart",
    "mark": "işaret",
    "market": "Market",
    "marketing": "pazarlama",
    "marriage": "evlilik",
    "married": "evli",
    "marry": "evlenmek",
    "mass": "kitle",
    "massive": "masif",
    "master": "usta",
    "match": "eşleşme",
    "matching": "eşleştirme",
    "match up": "eşleştir",
    "mate": "Dostum",
    "material": "malzeme",
    "mathematics": "matematik",
    "matter": "Önemli olmak",
    "maximum": "maksimum",
    "may": "Mayıs",
    "May": "Mayıs",
    "maybe": "olabilir",
    "mayor": "Belediye Başkanı",
    "me": "ben mi",
    "meal": "yemek",
    "mean": "anlamına gelmek",
    "meaning": "anlamı",
    "means": "anlamına geliyor",
    "meanwhile": "o esnada",
    "measure": "tedbir",
    "measurement": "ölçüm",
    "meat": "et",
    "media": "medya",
    "medical": "tıbbi",
    "medicine": "ilaç",
    "medium": "orta",
    "meet": "karşılamak",
    "meeting": "toplantı",
    "meet up": "buluşmak",
    "meet with": "ile bir araya",
    "melt": "erimek",
    "member": "üye",
    "membership": "üyelik",
    "memory": "hafıza",
    "mental": "zihinsel",
    "mentally": "zihinsel olarak",
    "mention": "Anma",
    "menu": "Menü",
    "mere": "sırf",
    "merely": "sadece",
    "mess": "dağınıklık",
    "message": "İleti",
    "metal": "metal",
    "method": "yöntem",
    "metre": "metre",
    "mid-": "orta",
    "midday": "öğlen",
    "middle": "orta",
    "midnight": "gece yarısı",
    "might": "belki",
    "mild": "hafif",
    "mile": "mil",
    "military": "askeri",
    "milk": "Süt",
    "milligram": "miligram",
    "millimetre": "milimetre",
    "million": "milyon",
    "millionth": "milyonuncu",
    "mind": "zihin",
    "mine": "benim",
    "mineral": "mineral",
    "minimum": "asgari",
    "minister": "bakan",
    "ministry": "bakanlık",
    "minor": "küçük",
    "minority": "azınlık",
    "minute 1": "dakika 1",
    "mirror": "ayna",
    "miss": "Özlemek",
    "missing": "eksik",
    "miss out": "kaçırmak",
    "mistake": "hata",
    "mistake for": "için hata",
    "mistaken": "yanlış",
    "mix": "mix",
    "mixed": "karışık",
    "mixture": "karışım",
    "mix up": "karıştır",
    "mobile": "seyyar",
    "mobile phone": "cep telefonu",
    "model": "model",
    "modern": "modern",
    "mom": "anne",
    "moment": "an",
    "Monday": "Pazartesi",
    "money": "para",
    "monitor": "izlemek",
    "month": "ay",
    "mood": "ruh hali",
    "moon": "ay",
    "moral": "manevi",
    "morally": "manevi olarak",
    "more": "Daha",
    "moreover": "Dahası",
    "morning": "sabah",
    "most": "çoğu",
    "mostly": "çoğunlukla",
    "mother": "anne",
    "motion": "hareket",
    "motor": "motor",
    "motorbike": "motosiklet",
    "motorcycle": "motosiklet",
    "mount": "dağ",
    "mountain": "dağ",
    "mouse": "fare",
    "mouth": "ağız",
    "move": "hareket",
    "move in": "taşınmak",
    "movement": "hareket",
    "move out": "taşınmak",
    "move over": "üzerinde hareket",
    "movie": "film",
    "movie theater": "sinema",
    "moving": "hareketli",
    "Mr": "Bay",
    "Mrs": "Bayan",
    "Ms": "Hanım",
    "much": "çok",
    "mud": "çamur",
    "multiply": "çarpmak",
    "mum": "anne",
    "murder": "cinayet",
    "muscle": "kas",
    "museum": "müze",
    "music": "müzik",
    "musical": "müzikal",
    "musician": "müzisyen",
    "must": "zorunlu",
    "my": "benim",
    "myself": "kendim",
    "mysterious": "gizemli",
    "mystery": "gizem",
    "nail": "tırnak",
    "naked": "çıplak",
    "name": "isim",
    "narrow": "dar",
    "nation": "ulus",
    "national": "Ulusal",
    "natural": "doğal",
    "naturally": "doğal olarak",
    "nature": "doğa",
    "navy": "Donanma",
    "near": "yakın",
    "nearby": "yakında",
    "nearly": "neredeyse",
    "neat": "temiz",
    "neatly": "düzgün şekilde",
    "necessarily": "zorunlu olarak",
    "necessary": "gerekli",
    "neck": "boyun",
    "need": "ihtiyaç",
    "needle": "iğne",
    "negative": "olumsuz",
    "neighbour": "komşu",
    "neighbourhood": "Komşuluk",
    "neither": "ne",
    "nephew": "erkek yeğen",
    "nerve": "sinir",
    "nervous": "sinirli",
    "nervously": "sinirli olarak",
    "nest": "yuva",
    "net": "ağ",
    "network": "ağ",
    "never": "asla",
    "nevertheless": "yine de",
    "new": "yeni",
    "newly": "yeni",
    "news": "Haberler",
    "newspaper": "gazete",
    "next": "Sonraki",
    "next to": "bitişik, yanında",
    "nice": "Güzel",
    "nicely": "güzelce",
    "niece": "yeğen",
    "night": "gece",
    "nine": "dokuz",
    "nineteen": "on dokuz",
    "nineteenth": "on dokuzuncu",
    "ninetieth": "doksanıncı",
    "ninety": "doksan",
    "ninth": "dokuzuncu",
    "no": "Hayır",
    "nobody": "kimse",
    "noise": "gürültü, ses",
    "noisily": "gürültüyle",
    "noisy": "gürültülü",
    "non-": "olmayan",
    "none": "Yok",
    "nonsense": "saçmalık",
    "no one": "hiç kimse",
    "nor": "ne de",
    "normal": "normal",
    "normally": "normalde",
    "north": "kuzeyinde",
    "northern": "kuzey",
    "nose": "burun",
    "not": "değil",
    "note": "Not",
    "note down": "not almak",
    "nothing": "hiçbir şey değil",
    "notice": "farkına varmak",
    "noticeable": "farkedilebilir",
    "novel": "Roman",
    "November": "Kasım",
    "now": "şimdi",
    "nowhere": "Hiçbir yerde",
    "nuclear": "nükleer",
    "number": "numara",
    "nurse": "hemşire",
    "nut": "fındık",
    "obey": "itaat etmek",
    "object": "nesne",
    "objective": "amaç",
    "observation": "gözlem",
    "observe": "gözlemek",
    "obtain": "elde etmek",
    "obvious": "açık",
    "obviously": "belli ki",
    "occasion": "fırsat",
    "occasionally": "bazen",
    "occupied": "meşgul",
    "occupy": "işgal etmek",
    "occur": "meydana",
    "occur to": "meydana",
    "ocean": "okyanus",
    "o’clock": "saat",
    "October": "Ekim",
    "odd": "garip",
    "oddly": "tuhaf bir şekilde",
    "of": "nın-nin",
    "off": "kapalı",
    "offence": "suç",
    "offend": "gücendirmek",
    "offense": "suç",
    "offensive": "saldırgan",
    "offer": "teklif",
    "office": "ofis",
    "officer": "subay",
    "official": "resmi",
    "officially": "resmi olarak",
    "often": "sıklıkla",
    "oh": "aman",
    "oil": "sıvı yağ",
    "OK": "TAMAM MI",
    "old": "eski",
    "old-fashioned": "eski moda",
    "on": "üzerinde",
    "once": "bir Zamanlar",
    "one": "bir",
    "one another": "bir başka",
    "onion": "soğan",
    "online": "internet üzerinden",
    "only": "sadece",
    "onto": "üstüne",
    "open": "açık",
    "opening": "açılış",
    "openly": "açıkça",
    "open up": "açmak",
    "operate": "işletmek",
    "operation": "operasyon",
    "opinion": "görüş",
    "opponent": "karşı taraf",
    "opportunity": "fırsat",
    "oppose": "karşı çıkmak",
    "opposed": "zıt",
    "opposing": "karşı",
    "opposite": "karşısında",
    "opposition": "muhalefet",
    "option": "seçenek",
    "or": "veya",
    "orange": "Portakal",
    "order": "sipariş",
    "ordinary": "sıradan",
    "organ": "organ",
    "organization": "organizasyon",
    "organize": "düzenlemek",
    "organized": "örgütlü",
    "origin": "Menşei",
    "original": "orijinal",
    "originally": "aslında",
    "other": "diğer",
    "otherwise": "aksi takdirde",
    "ought to": "gerektiğini",
    "our": "bizim",
    "ours": "bizim",
    "ourselves": "kendimizi",
    "out": "dışarı",
    "outdoor": "dış mekan",
    "outdoors": "açık havada",
    "outer": "dış",
    "outline": "taslak",
    "output": "çıktı",
    "outside": "dışarıda",
    "outstanding": "Muhteşem",
    "oven": "fırın",
    "over": "bitmiş",
    "overall": "tüm",
    "overcome": "aşmak",
    "owe": "borçlu olmak",
    "own": "kendi",
    "owner": "sahip",
    "own up": "sahiplenmek",
    "pace 1": "tempo 1",
    "pack": "pack",
    "package": "paket",
    "packaging": "paketleme",
    "packet": "paket",
    "pack up": "toparlanmak",
    "page": "sayfa",
    "pain": "Ağrı",
    "painful": "acı verici",
    "paint": "boya",
    "painter": "ressam",
    "painting": "boyama",
    "pair": "çift",
    "palace": "Saray",
    "pale": "soluk",
    "pan 1": "1 kaydırmak",
    "panel": "panel",
    "pants": "pantolon",
    "paper": "kâğıt",
    "parallel": "paralel",
    "parent": "ebeveyn",
    "park": "park",
    "parliament": "parlamento",
    "part": "Bölüm",
    "particular": "belirli",
    "particularly": "özellikle",
    "partly": "kısmen",
    "partner": "ortak",
    "partnership": "ortaklık",
    "party": "Parti",
    "pass": "geçmek",
    "passage": "geçit",
    "pass away": "vefat etmek",
    "pass by": "geçmek",
    "passenger": "yolcu",
    "passing": "geçen",
    "pass on": "geçmek",
    "pass out": "bayılmak",
    "passport": "pasaport",
    "pass round": "yuvarlak geçmesi",
    "pass through": "içinden geçmek",
    "past": "geçmiş",
    "path": "yol",
    "patience": "sabır",
    "patient": "hasta",
    "pattern": "Desen",
    "pause": "Duraklat",
    "pay": "ödemek",
    "pay back": "geri ödeme",
    "payment": "ödeme",
    "pay out": "ödemek",
    "pay up": "tamamen ödemek",
    "peace": "Barış",
    "peaceful": "huzurlu",
    "peak": "zirve",
    "pen": "dolma kalem",
    "pencil": "kalem",
    "penny": "kuruş",
    "pension 1": "emeklilik 1",
    "people": "insanlar",
    "pepper": "biber",
    "per": "başına",
    "per cent": "yüzde",
    "perfect": "mükemmel",
    "perfectly": "mükemmel bir şekilde",
    "perform": "yapmak",
    "performance": "verim",
    "performer": "sanatçı",
    "perhaps": "belki",
    "period": "dönem",
    "permanent": "kalıcı",
    "permanently": "kalıcı olarak",
    "permission": "izin",
    "permit": "izin",
    "person": "kişi",
    "personal": "kişiye özel",
    "personality": "kişilik",
    "personally": "Şahsen",
    "persuade": "ikna etmek",
    "pet": "Evcil Hayvan",
    "petrol": "benzin",
    "phase": "evre",
    "philosophy": "Felsefe",
    "phone": "telefon",
    "photo": "Fotoğraf",
    "photocopy": "fotokopi",
    "photograph": "fotoğraf",
    "photographer": "fotoğrafçı",
    "photography": "fotoğrafçılık",
    "phrase": "ifade",
    "physical": "fiziksel",
    "physically": "fiziksel olarak",
    "physics": "fizik",
    "piano": "piyano",
    "pick": "toplamak",
    "pick up": "almak",
    "picture": "resim",
    "piece": "parça",
    "pig": "domuz",
    "pile": "istif",
    "pile up": "yığmak",
    "pill": "hap",
    "pilot": "pilot",
    "pin": "toplu iğne",
    "pink": "pembe",
    "pint": "pint",
    "pipe": "boru",
    "pitch": "Saha",
    "pity": "yazık",
    "place": "yer",
    "plain": "sade",
    "plan": "plan",
    "plane": "uçak",
    "planet": "gezegen",
    "planning": "planlama",
    "plant": "bitki",
    "plastic": "plastik",
    "plate": "tabak",
    "platform": "platform",
    "play": "Oyna",
    "play about": "yaklaşık oynamak",
    "player": "oyuncu",
    "play with": "ile oyna",
    "pleasant": "hoş",
    "pleasantly": "hoş",
    "please": "Lütfen",
    "pleased": "memnun",
    "pleasing": "hoş",
    "pleasure": "Zevk",
    "plenty": "bol",
    "plot": "arsa",
    "plug": "fiş",
    "plug in": "Eklenti",
    "plus 1": "artı 1",
    "p.m.": "öğleden sonra",
    "pocket": "cep",
    "poem": "şiir",
    "poetry": "şiir",
    "point": "nokta",
    "pointed": "işaretlendi",
    "point out": "işaret etmek",
    "poison": "zehir",
    "poisonous": "zehirli",
    "pole": "kutup",
    "police": "polis",
    "policy": "politika",
    "polish": "Lehçe",
    "polite": "kibar",
    "politely": "kibarca",
    "political": "siyasi",
    "politically": "politik olarak",
    "politician": "politikacı",
    "politics": "siyaset",
    "pollution": "kirlilik",
    "pool": "havuz",
    "poor": "yoksul",
    "pop": "pop",
    "popular": "popüler",
    "population": "nüfus",
    "port": "Liman",
    "pose": "poz",
    "position": "durum",
    "positive": "pozitif",
    "possess": "sahip olmak",
    "possession": "kontrol altına alma",
    "possibility": "olasılık",
    "possible": "mümkün",
    "possibly": "belki",
    "post": "İleti",
    "post office": "Postane",
    "pot": "tencere",
    "potato": "Patates",
    "potential": "potansiyel",
    "potentially": "potansiyel",
    "pound": "pound",
    "pour": "dökün",
    "powder": "pudra",
    "power": "güç",
    "powerful": "güçlü",
    "practical": "pratik",
    "practically": "pratikte",
    "practice": "uygulama",
    "practise": "uygulama",
    "praise": "övgü",
    "pray": "dua etmek",
    "prayer": "namaz",
    "precise": "kesin",
    "precisely": "tam",
    "predict": "tahmin",
    "prefer": "tercih etmek",
    "preference": "tercih",
    "pregnant": "hamile",
    "premises": "tesislerinde",
    "preparation": "hazırlık",
    "prepare": "Hazırlamak",
    "prepared": "hazırlanmış",
    "presence": "varlık",
    "present": "mevcut",
    "presentation": "sunum",
    "preserve": "korumak",
    "president": "Devlet Başkanı",
    "press": "basın",
    "pressure": "basınç",
    "presumably": "muhtemelen",
    "pretend": "taklit",
    "pretty": "güzel",
    "prevent": "önlemek",
    "previous": "önceki",
    "previously": "Önceden",
    "price": "fiyat",
    "pride": "gurur",
    "priest": "rahip",
    "primarily": "öncelikle",
    "primary": "birincil",
    "prime minister": "Başbakan",
    "prince": "prens",
    "princess": "prenses",
    "principle": "prensip",
    "print": "Yazdır",
    "printer": "yazıcı",
    "printing": "baskı",
    "print off": "negatiften çıkarmak",
    "prior": "önceki",
    "priority": "öncelik",
    "prison": "hapis",
    "prisoner": "mahkum",
    "private": "özel",
    "privately": "özel",
    "prize": "ödül",
    "probable": "muhtemel",
    "probably": "muhtemelen",
    "problem": "sorun",
    "procedure": "prosedür",
    "proceed": "ilerlemek",
    "process 1": "proses 1",
    "produce": "üretmek",
    "producer": "üretici",
    "product": "ürün",
    "production": "üretim",
    "profession": "meslek",
    "professional": "profesyonel",
    "professor": "profesör",
    "profit": "kâr",
    "program": "program",
    "programme": "program",
    "progress": "ilerleme",
    "project": "proje",
    "promise": "söz vermek",
    "promote": "desteklemek",
    "promotion": "tanıtım",
    "prompt": "Komut istemi",
    "promptly": "derhal",
    "pronounce": "telaffuz",
    "pronunciation": "telaffuz",
    "proof": "kanıt",
    "proper": "uygun",
    "properly": "uygun şekilde",
    "property": "Emlak",
    "proportion": "oran",
    "proposal": "öneri",
    "propose": "teklif etmek, önermek",
    "prospect": "olasılık",
    "protect": "korumak",
    "protection": "koruma",
    "protest": "protesto",
    "proud": "gururlu",
    "proudly": "gururla",
    "prove": "kanıtlamak",
    "provide": "sağlamak",
    "provided": "sağlanan",
    "providing": "sağlama",
    "pub": "birahane",
    "public": "halka açık",
    "publication": "yayın",
    "publicity": "tanıtım",
    "publicly": "alenen",
    "publish": "Yayınla",
    "publishing": "yayıncılık",
    "pull": "Çek",
    "pull apart": "çekme dışında",
    "pull down": "aşağı çek",
    "pull in": "içeri çekmek",
    "pull off": "koparmak",
    "pull out": "çıkarmak",
    "pull over": "kenara çekmek",
    "pull through": "yakayı sıyırmak",
    "pull together": "elbirliği ile çalışmak",
    "pull up": "yukarı çekmek",
    "punch": "yumruk",
    "punish": "cezalandırmak",
    "punishment": "ceza",
    "pupil": "öğrenci",
    "purchase": "satın alma",
    "pure": "saf",
    "purely": "yalnızca",
    "purple": "mor",
    "purpose": "amaç",
    "pursue": "sürdürmek",
    "push": "it",
    "push about": "yaklaşık itmek",
    "push forward": "ilerletmek",
    "put": "koymak",
    "put away": "Bırakmak",
    "put back": "geri koy",
    "put down": "bastırmak",
    "put forward": "ileri sürmek",
    "put in": "koymak",
    "put off": "söndürmek",
    "put on": "giymek",
    "put out": "söndürmek",
    "put through": "bağlamak",
    "put together": "araya",
    "put up": "çıkarmak",
    "put up with": "katlanmak",
    "qualification": "vasıf",
    "qualified": "nitelikli",
    "qualify": "nitelemek",
    "quality": "kalite",
    "quantity": "miktar",
    "quarter": "çeyrek",
    "queen": "kraliçe",
    "question": "soru",
    "quick": "hızlı",
    "quickly": "hızlı bir şekilde",
    "quiet": "sessiz",
    "quietly": "sessizce",
    "quit": "çıkmak",
    "quite": "epeyce",
    "quote": "alıntı",
    "race": "yarış",
    "racing": "yarış",
    "radio": "radyo",
    "rail": "Demiryolu",
    "railroad": "demiryolu",
    "railway": "demiryolu",
    "rain": "yağmur",
    "raise": "yükseltmek",
    "range": "Aralık",
    "rank": "rütbe",
    "rapid": "hızlı",
    "rapidly": "hızla",
    "rare": "nadir",
    "rarely": "seyrek",
    "rate": "oran",
    "rather": "daha doğrusu",
    "raw": "çiğ",
    "re-": "yeniden",
    "reach": "uzanma",
    "react": "tepki",
    "reaction": "reaksiyon",
    "read": "okumak",
    "reader": "okuyucu",
    "reading": "okuma",
    "read out": "okuma",
    "read over": "üzerinde okunan",
    "ready": "hazır",
    "real": "gerçek",
    "realistic": "gerçekçi",
    "reality": "gerçeklik",
    "realize": "Fark",
    "really": "Gerçekten mi",
    "rear": "arka",
    "reason": "neden",
    "reasonable": "makul",
    "reasonably": "oldukça",
    "recall": "hatırlama",
    "receipt": "fiş",
    "receive": "teslim almak",
    "recent": "son",
    "recently": "son günlerde",
    "reception": "resepsiyon",
    "reckon": "Hesaba katmak",
    "reckon on": "güvenmek",
    "recognition": "tanıma",
    "recognize": "tanımak",
    "recommend": "önermek",
    "record": "kayıt",
    "recording": "kayıt",
    "recover": "kurtarmak",
    "red": "kırmızı",
    "reduce": "azaltmak",
    "reduction": "indirgeme",
    "refer": "başvurmak",
    "reference": "referans",
    "refer to": "bakın",
    "reflect": "yansıtmak",
    "reform": "reform",
    "refrigerator": "buzdolabı",
    "refusal": "ret",
    "refuse 1": "1 çöp",
    "regard": "saygı",
    "regarding": "ilişkin",
    "region": "bölge",
    "regional": "bölgesel",
    "register": "Kayıt ol",
    "regret": "pişmanlık",
    "regular": "düzenli",
    "regularly": "düzenli olarak",
    "regulation": "düzenleme",
    "reject": "reddetmek",
    "relate": "ilgili olmak",
    "related": "ilişkili",
    "relate to": "ilgili",
    "relation": "ilişki",
    "relationship": "ilişki",
    "relative": "bağıl",
    "relatively": "Nispeten",
    "relax": "Rahatlayın",
    "relaxed": "rahat",
    "relaxing": "rahatlatıcı",
    "release": "serbest bırakmak",
    "relevant": "uygun",
    "relief": "Rahatlama",
    "religion": "din",
    "religious": "dini",
    "rely": "güvenmek",
    "rely on": "güvenmek",
    "remain": "kalmak",
    "remaining": "kalan",
    "remains": "kalıntılar",
    "remark": "düşünce",
    "remarkable": "dikkat çekici",
    "remarkably": "oldukça",
    "remember": "hatırlamak",
    "remind": "hatırlatmak",
    "remind of": "hatırlatmak",
    "remote": "uzak",
    "removal": "uzaklaştırma",
    "remove": "Kaldır",
    "rent": "kira",
    "rented": "kiralanmış",
    "repair": "tamir etmek",
    "repeat": "tekrar et",
    "repeated": "tekrarlanan",
    "repeatedly": "defalarca",
    "replace": "değiştirmek",
    "reply": "cevap",
    "report": "bildiri",
    "represent": "temsil etmek",
    "representative": "temsilci",
    "reproduce": "üretmesidir",
    "reputation": "itibar",
    "request": "istek",
    "require": "gerektirir",
    "requirement": "gereklilik",
    "rescue": "kurtarmak",
    "research": "Araştırma",
    "reservation": "rezervasyon",
    "reserve": "rezerv",
    "resident": "oturan",
    "resist": "direnmek",
    "resistance": "direnç",
    "resolve": "çözmek",
    "resort": "dinlenme tesisi",
    "resort to": "başvurmak",
    "resource": "kaynak",
    "respect": "saygı",
    "respond": "cevap vermek",
    "response": "tepki",
    "responsibility": "sorumluluk",
    "responsible": "sorumluluk sahibi",
    "rest": "dinlenme",
    "restaurant": "restoran",
    "restore": "onarmak",
    "restrict": "kısıtlamak",
    "restricted": "kısıtlı",
    "restriction": "kısıtlama",
    "result": "sonuç",
    "result in": "sonuçlanmak",
    "retain": "tutmak",
    "retire": "emekli olmak",
    "retired": "emekli",
    "retirement": "emeklilik",
    "return": "dönüş",
    "reveal": "ortaya çıkartmak",
    "reverse": "tersine çevirmek",
    "review": "gözden geçirmek",
    "revise": "tashih",
    "revision": "revizyon",
    "revolution": "devrim",
    "reward": "ödül",
    "rhythm": "ritim",
    "rice": "pirinç",
    "rich": "zengin",
    "rid": "kurtulmak",
    "ride": "binmek",
    "rider": "binici",
    "ridiculous": "saçma",
    "riding": "binme",
    "right": "sağ",
    "rightly": "haklı olarak",
    "ring 1": "halka 1",
    "ring 2": "halka 2",
    "ring back": "halka sırt",
    "rise": "yükselmek",
    "risk": "risk",
    "rival": "rakip",
    "river": "nehir",
    "road": "yol",
    "rob": "soymak",
    "rock": "Kaya",
    "role": "rol",
    "roll": "rulo",
    "romantic": "romantik",
    "roof": "çatı",
    "room": "oda",
    "root": "kök",
    "rope": "İp",
    "rough": "kaba",
    "roughly": "kabaca",
    "round": "yuvarlak",
    "rounded": "yuvarlak",
    "route": "rota",
    "routine": "rutin",
    "row 1": "satır 1",
    "royal": "Kraliyet",
    "rub": "ovmak",
    "rubber": "silgi",
    "rubbish": "çöp",
    "rude": "kaba",
    "rudely": "terbiyesizce",
    "ruin": "harabe",
    "ruined": "harap",
    "rule": "kural",
    "rule out": "kural dışı",
    "ruler": "cetvel",
    "rumour": "söylenti",
    "run": "Çalıştırmak",
    "run after": "peşinden koşmak",
    "run away": "Kaçmak",
    "runner": "koşucu",
    "running": "çalışan",
    "run out": "dışarı koşmak",
    "run over": "ezmek",
    "run through": "hızlıca gözden geçirme",
    "rural": "kırsal",
    "rush": "acele",
    "sack": "çuval",
    "sad": "üzgün",
    "sadly": "ne yazık ki",
    "sadness": "üzüntü",
    "safe": "kasa",
    "safely": "güvenli bir şekilde",
    "safety": "Emniyet",
    "sail": "yelken",
    "sailing": "yelkencilik",
    "sailor": "denizci",
    "salad": "salata",
    "salary": "maaş",
    "sale": "satış",
    "salt": "tuz",
    "salty": "tuzlu",
    "same": "aynı",
    "sample": "örneklem",
    "sand": "kum",
    "satisfaction": "memnuniyet",
    "satisfied": "memnun",
    "satisfy": "tatmin etmek",
    "satisfying": "doyurucu",
    "Saturday": "Cumartesi",
    "sauce": "Sos",
    "save": "kayıt etmek",
    "saving": "tasarruf",
    "say": "söyle",
    "scale": "ölçek",
    "scare": "Korkmak",
    "scared": "korkuttu",
    "scare off": "korkutmak",
    "scene": "faliyet alani, sahne",
    "schedule": "program",
    "scheme": "düzen",
    "school": "okul",
    "science": "Bilim",
    "scientific": "ilmi",
    "scientist": "Bilim insanı",
    "scissors": "makas",
    "score": "Puan",
    "scratch": "kaşımak",
    "scream": "çığlık",
    "screen": "ekran",
    "screw": "vida",
    "sea": "deniz",
    "seal": "mühür",
    "seal off": "kapatmak",
    "search": "arama",
    "season": "mevsim",
    "seat": "oturma yeri",
    "second 1": "ikinci 1",
    "secondary": "ikincil",
    "secret": "gizli",
    "secretary": "Sekreter",
    "secretly": "gizlice",
    "section": "Bölüm",
    "sector": "sektör",
    "secure": "güvenli",
    "security": "güvenlik",
    "see": "görmek",
    "see about": "hakkında bkz",
    "seed": "tohum",
    "seek": "aramak",
    "seem": "görünmek",
    "see to": "görmek",
    "select": "seçmek",
    "selection": "seçim",
    "self": "öz",
    "self-": "öz",
    "sell": "satmak",
    "sell off": "elden çıkarmak",
    "sell out": "tasfiye etmek",
    "senate": "senato",
    "senator": "senatör",
    "send": "gönderme",
    "send for": "gönderilen",
    "send off": "göndermek",
    "senior": "kıdemli",
    "sense": "duyu",
    "sensible": "mantıklı",
    "sensitive": "hassas",
    "sentence": "cümle",
    "separate": "ayrı",
    "separated": "ayrıldı",
    "separately": "ayrı ayrı",
    "separation": "ayırma",
    "September": "Eylül",
    "series": "dizi",
    "serious": "ciddi",
    "seriously": "ciddi anlamda",
    "servant": "hizmetçi",
    "serve": "servis",
    "service": "hizmet",
    "session": "oturum, toplantı, celse",
    "set": "Ayarlamak",
    "set off": "ateşlemek",
    "set out": "yola çıkmak",
    "settle": "yerleşmek",
    "settle down": "yerleşmek",
    "set up": "kurulum",
    "seven": "Yedi",
    "seventeen": "on yedi",
    "seventh": "yedinci",
    "seventieth": "yetmişinci",
    "seventy": "yetmiş",
    "several": "birkaç",
    "severe": "şiddetli",
    "severely": "şiddetli",
    "sew": "dikmek",
    "sewing": "dikiş",
    "sex": "seks",
    "sexual": "cinsel",
    "sexually": "cinsel",
    "shade": "gölge",
    "shadow": "gölge",
    "shake": "sallamak",
    "shall": "acak",
    "shallow": "sığ",
    "shame": "utanç",
    "shape": "şekil",
    "shaped": "şekilli",
    "share": "Paylaş",
    "sharp": "keskin",
    "sharply": "keskin",
    "shave": "tıraş",
    "she": "o",
    "sheep": "koyun",
    "sheet": "tabaka",
    "shelf": "raf",
    "shell": "kabuk",
    "shelter": "barınak",
    "shift": "vardiya",
    "shine": "parlamak",
    "shiny": "parlak",
    "ship": "gemi",
    "shirt": "gömlek",
    "shock": "şok",
    "shocked": "şok",
    "shocking": "şok edici",
    "shoe": "ayakkabı",
    "shoot": "ateş etmek",
    "shoot down": "kapat",
    "shooting": "çekim",
    "shop": "Dükkan",
    "shopping": "alışveriş yapmak",
    "short": "kısa",
    "shortly": "kısaca",
    "shot": "atış",
    "should": "meli",
    "shoulder": "omuz",
    "shout": "bağırmak",
    "show": "göstermek",
    "shower": "duş",
    "show off": "hava atmak",
    "show round": "gösteri yuvarlak",
    "show up": "ortaya çıkmak",
    "shut": "kapamak",
    "shut down": "kapat",
    "shut in": "kapamak",
    "shut out": "girmesini engellemek",
    "shut up": "kapa çeneni",
    "shy": "utangaç",
    "sick": "hasta",
    "side": "yan",
    "sideways": "yan yan",
    "sight": "görme",
    "sign": "işaret",
    "signal": "işaret",
    "signature": "imza",
    "significant": "önemli",
    "significantly": "önemli ölçüde",
    "silence": "Sessizlik",
    "silent": "sessiz",
    "silk": "ipek",
    "silly": "saçma",
    "silver": "gümüş",
    "similar": "benzer",
    "similarly": "benzer şekilde",
    "simple": "basit",
    "simply": "basitçe",
    "since": "dan beri",
    "sincere": "samimi",
    "sincerely": "içtenlikle",
    "sing": "şarkı söyle",
    "singer": "şarkıcı",
    "singing": "Şarkı söyleme",
    "single": "tek",
    "sink": "lavabo",
    "sir": "Bayım",
    "sister": "kız kardeş",
    "sit": "sit",
    "sit down": "oturmak",
    "site": "yer",
    "situation": "durum",
    "six": "altı",
    "sixteen": "on altı",
    "sixth": "altıncı",
    "sixtieth": "altmışıncı",
    "sixty": "altmış",
    "size": "boyut",
    "skilful": "becerikli",
    "skilfully": "ustalıkla",
    "skill": "beceri",
    "skilled": "yetenekli",
    "skin": "cilt",
    "skirt": "etek",
    "sky": "gökyüzü",
    "sleep": "uyku",
    "sleeve": "kol",
    "slice": "dilim",
    "slide": "kaymak",
    "slight": "hafif",
    "slightly": "hafifçe",
    "slip": "kayma",
    "slope": "eğim",
    "slow": "yavaş",
    "slowly": "yavaşça",
    "small": "küçük",
    "smart": "akıllı",
    "smash": "parçalamak",
    "smell": "koku",
    "smile": "gülümsemek",
    "smoke": "Sigara içmek",
    "smoking": "sigara içmek",
    "smooth": "pürüzsüz",
    "smoothly": "düzgünce",
    "snake": "yılan",
    "snow": "kar",
    "so": "yani",
    "soap": "sabun",
    "social": "sosyal",
    "socially": "sosyal olarak",
    "society": "toplum",
    "sock": "çorap",
    "soft": "yumuşak",
    "softly": "usulca",
    "software": "yazılım",
    "soil": "toprak",
    "soldier": "asker",
    "solid": "katı",
    "solution": "çözüm",
    "solve": "çözmek",
    "some": "biraz",
    "somebody": "birisi",
    "somehow": "bir şekilde",
    "someone": "birisi",
    "something": "bir şey",
    "sometimes": "ara sıra",
    "somewhat": "biraz",
    "somewhere": "bir yerde",
    "son": "oğul",
    "song": "şarkı",
    "soon": "yakında",
    "sore": "Ağrı",
    "sorry": "afedersiniz",
    "sort": "çeşit",
    "sort out": "problemi çözmek",
    "soul": "ruh",
    "sound": "ses",
    "soup": "çorba",
    "sour": "Ekşi",
    "source": "kaynak",
    "south": "güney",
    "southern": "güney",
    "space": "Uzay",
    "spare": "yedek",
    "speak": "konuşması",
    "speaker": "konuşmacı",
    "speak out": "sesli söyle",
    "speak up": "konuş",
    "special": "özel",
    "specialist": "uzman",
    "specially": "özel olarak",
    "specific": "özel",
    "specifically": "özellikle",
    "speech": "konuşma",
    "speed": "hız",
    "speed up": "hızlandırmak",
    "spell": "harf harf kodlamak",
    "spelling": "yazım",
    "spend": "harcamak",
    "spice": "baharat",
    "spicy": "baharatlı",
    "spider": "örümcek",
    "spin": "çevirmek",
    "spirit": "ruh",
    "spiritual": "manevi",
    "spite": "nispet",
    "split": "Bölünmüş",
    "split up": "ayrılmak",
    "spoil": "Yağlamalamak",
    "spoken": "konuşulmuş",
    "spoon": "kaşık",
    "sport": "spor",
    "spot": "yer",
    "spray": "sprey",
    "spread": "yayılmış",
    "spread out": "yayılmak",
    "spring": "bahar",
    "square": "Meydan",
    "squeeze": "suyunu sıkmak",
    "stable": "kararlı",
    "staff": "Personel",
    "stage": "sahne",
    "stair": "basamak",
    "stamp": "pul",
    "stand": "ayakta durmak",
    "standard": "standart",
    "stand back": "geri çekil",
    "stand by": "yanında olmak",
    "stand for": "ayakta",
    "stand out": "dikkat çekmek",
    "stand up": "ayağa kalk",
    "stand up for": "desteklemek",
    "star": "star",
    "stare": "gözünü dikmek",
    "start": "Başlat",
    "start off": "başlamak",
    "start out": "işe koyulmak",
    "start up": "başlamak",
    "state": "durum",
    "statement": "Beyan",
    "station": "istasyon",
    "statue": "heykel",
    "status": "durum",
    "stay": "kalmak",
    "stay away": "uzak dur",
    "stay out of": "dışında kalmak",
    "steadily": "sürekli",
    "steady": "istikrarlı",
    "steal": "çalmak",
    "steam": "buhar",
    "steel": "çelik",
    "steep": "dik",
    "steeply": "dik",
    "steer": "direksiyon",
    "step": "adım",
    "stick": "Çubuk",
    "stick out": "diretmek",
    "stick to": "için sopa",
    "stick up": "soymak",
    "sticky": "yapışkan",
    "stiff": "katı",
    "stiffly": "kaskatı",
    "still": "yine",
    "sting": "acı",
    "stir": "karıştırmak",
    "stock": "Stok",
    "stomach": "mide",
    "stone": "taş",
    "stop": "Dur",
    "store": "mağaza",
    "storm": "fırtına",
    "story": "hikaye",
    "stove": "soba, fırın, ocak",
    "straight": "Düz",
    "strain": "Gerginlik",
    "strange": "garip",
    "strangely": "garip biçimde",
    "stranger": "yabancı",
    "strategy": "strateji",
    "stream": "Akış",
    "street": "sokak",
    "strength": "kuvvet",
    "stress": "stres",
    "stressed": "stresli",
    "stretch": "Uzatmak",
    "strict": "sıkı",
    "strictly": "kesinlikle",
    "strike": "vuruş",
    "striking": "dikkat çekici",
    "string": "sicim",
    "strip": "şerit",
    "stripe": "şerit",
    "striped": "çizgili",
    "stroke": "inme",
    "strong": "kuvvetli",
    "strongly": "şiddetle",
    "structure": "yapı",
    "struggle": "mücadele etmek",
    "student": "Öğrenci",
    "studio": "stüdyo",
    "study": "ders çalışma",
    "stuff": "şey",
    "stupid": "Aptal",
    "style": "stil",
    "subject": "konu",
    "substance": "madde",
    "substantial": "önemli",
    "substantially": "esasen",
    "substitute": "vekil",
    "succeed": "başarılı olmak",
    "success": "başarı",
    "successful": "başarılı",
    "successfully": "başarılı olarak",
    "such": "böyle",
    "suck": "emmek",
    "sudden": "ani",
    "suddenly": "aniden",
    "suffer": "acı çekmek",
    "suffering": "çile",
    "sufficient": "yeterli",
    "sufficiently": "yeteri kadar",
    "sugar": "şeker",
    "suggest": "önermek",
    "suggestion": "öneri",
    "suit": "takım elbise",
    "suitable": "uygun",
    "suitcase": "bavul",
    "suited": "uygun",
    "sum": "toplam",
    "summary": "özet",
    "summer": "yaz",
    "sum up": "Özetle",
    "sun": "Güneş",
    "Sunday": "Pazar",
    "superior": "üstün",
    "supermarket": "süpermarket",
    "supply": "arz",
    "support": "destek",
    "supporter": "destekçi",
    "suppose": "varsaymak",
    "sure": "Elbette",
    "surely": "elbette",
    "surface": "yüzey",
    "surname": "soyadı",
    "surprise": "sürpriz",
    "surprised": "sürpriz",
    "surprising": "şaşırtıcı",
    "surprisingly": "şaşırtıcı biçimde",
    "surround": "kuşatma",
    "surrounding": "çevreleyen",
    "surroundings": "çevre",
    "survey": "anket",
    "survive": "hayatta kalmak",
    "suspect": "şüpheli",
    "suspicion": "şüphe",
    "suspicious": "şüpheli",
    "swallow": "Yutmak",
    "swear": "yemin etmek",
    "swearing": "küfür",
    "sweat": "ter",
    "sweater": "Kazak",
    "sweep": "süpürme",
    "sweet": "tatlı",
    "swell": "kabarma",
    "swelling": "şişme",
    "swim": "yüzmek",
    "swimming": "yüzme",
    "swimming pool": "Yüzme havuzu",
    "swing": "salıncak",
    "switch": "değiştirmek",
    "switch off": "kapatmak",
    "swollen": "şişmiş",
    "symbol": "sembol",
    "sympathetic": "sempatik",
    "sympathy": "sempati",
    "system": "sistem",
    "table": "tablo",
    "tablet": "tablet",
    "tackle": "ele almak",
    "tail": "kuyruk",
    "take": "almak",
    "take away": "paket servisi",
    "take back": "geri al",
    "take down": "devirmek",
    "take in": "almak",
    "take off": "havalanmak",
    "take on": "üstüne almak",
    "take over": "Devralmak",
    "take up": "almak",
    "talk": "konuşma",
    "tall": "uzun boylu",
    "tank": "tank",
    "tap": "dokunmak",
    "tape": "bant",
    "target": "hedef",
    "task": "görev",
    "taste": "damak zevki",
    "tax": "vergi",
    "taxi": "taksi",
    "tea": "Çay",
    "teach": "öğretme",
    "teacher": "öğretmen",
    "teaching": "öğretim",
    "team": "takım",
    "tear 1": "yırtılma 1",
    "tear 2": "yırtılma 2",
    "tear up": "yırtıp atmak",
    "technical": "teknik",
    "technique": "teknik",
    "technology": "teknoloji",
    "telephone": "telefon",
    "television": "televizyon",
    "tell": "söylemek",
    "tell off": "azarlamak",
    "temperature": "sıcaklık",
    "temporarily": "geçici",
    "temporary": "geçici",
    "ten": "on",
    "tend": "Bakmak",
    "tendency": "eğilim",
    "tension": "gerginlik",
    "tent": "çadır",
    "tenth": "onuncu",
    "term": "terim",
    "terrible": "korkunç",
    "terribly": "son derece",
    "test": "Ölçek",
    "text": "Metin",
    "than": "göre",
    "thank": "teşekkür",
    "thanks": "Teşekkürler",
    "thank you": "teşekkür ederim",
    "that": "o",
    "the": "",
    "theatre": "tiyatro",
    "their": "onların",
    "theirs": "onların",
    "them": "onları",
    "theme": "tema",
    "themselves": "kendilerini",
    "then": "sonra",
    "theory": "teori",
    "there": "Orada",
    "therefore": "bu nedenle",
    "they": "onlar",
    "thick": "kalın",
    "thickly": "sık",
    "thickness": "kalınlık",
    "thief": "hırsız",
    "thin": "ince",
    "thing": "şey",
    "think": "düşünmek",
    "think about": "hakkında düşün",
    "thinking": "düşünme",
    "think of": "düşünmek",
    "think of as": "olarak düşünmek",
    "think over": "üzerinde düşünmek",
    "think up": "tasarlamak",
    "third": "üçüncü",
    "thirsty": "susuz",
    "thirteen": "on üç",
    "thirteenth": "on üçüncü",
    "thirtieth": "otuzuncu",
    "thirty": "otuz",
    "this": "bu",
    "thorough": "tam",
    "thoroughly": "iyice",
    "though": "rağmen",
    "thought": "düşünce",
    "thousand": "bin",
    "thousandth": "bininci",
    "thread": "Konu",
    "threat": "tehdit",
    "threaten": "tehdit etmek",
    "threatening": "tehdit",
    "three": "üç",
    "throat": "boğaz",
    "through": "vasıtasıyla",
    "throughout": "boyunca",
    "throw": "atmak",
    "throw away": "fırlatmak",
    "throw out": "dışarı atmak",
    "thumb": "başparmak",
    "Thursday": "Perşembe",
    "thus": "Böylece",
    "ticket": "bilet",
    "tidy": "düzenli",
    "tie": "kravat",
    "tie up": "Bağlamak",
    "tight": "sıkı",
    "tightly": "sıkıca",
    "till": "kadar",
    "time": "zaman",
    "timetable": "masa saati",
    "tin": "teneke",
    "tiny": "çok küçük",
    "tip": "İpucu",
    "tip over": "devrilmek",
    "tire": "tekerlek",
    "tired": "yorgun",
    "tire out": "canını çıkarmak",
    "tiring": "yorucu",
    "title": "Başlık",
    "to": "için",
    "today": "bugün",
    "toe": "ayak parmağı",
    "together": "birlikte",
    "toilet": "tuvalet",
    "tomato": "domates",
    "tomorrow": "yarın",
    "ton": "ton",
    "tone": "ton",
    "tongue": "dil",
    "tonight": "Bu gece",
    "tonne": "ton",
    "too": "çok",
    "tool": "araç",
    "tooth": "diş",
    "top": "üst",
    "topic": "konu",
    "total": "Toplam",
    "totally": "bütünüyle",
    "touch": "dokunma",
    "tough": "zorlu",
    "tour": "tur",
    "tourist": "turist",
    "towards": "karşı",
    "towel": "havlu",
    "tower": "kule",
    "town": "kasaba",
    "toy": "oyuncak",
    "trace": "iz",
    "track": "Izlemek",
    "trade": "Ticaret",
    "trading": "ticari",
    "tradition": "gelenek",
    "traditional": "geleneksel",
    "traditionally": "geleneksel",
    "traffic": "trafik",
    "train": "tren",
    "training": "Eğitim",
    "transfer": "Aktar",
    "transform": "dönüştürmek",
    "translate": "Çevirmek",
    "translation": "tercüme",
    "transparent": "şeffaf",
    "transport": "Ulaşım",
    "transportation": "taşımacılık",
    "trap": "tuzak",
    "travel": "seyahat",
    "traveller": "gezgin",
    "treat": "tedavi etmek",
    "treatment": "tedavi",
    "tree": "ağaç",
    "trend": "akım",
    "trial": "Deneme",
    "triangle": "üçgen",
    "trick": "hile",
    "trip": "gezi",
    "tropical": "tropikal",
    "trouble": "sorun",
    "trousers": "pantolon",
    "truck": "kamyon",
    "true": "true",
    "truly": "gerçekten",
    "trust": "güven",
    "truth": "hakikat",
    "try": "Deneyin",
    "try on": "denemek",
    "try out": "denemek",
    "tube": "tüp",
    "Tuesday": "Salı",
    "tune": "melodi",
    "tunnel": "tünel",
    "turn": "çevirmek",
    "turn back": "geri dön",
    "turn down": "kısmak",
    "turn into": "dönüşmek",
    "turn off": "kapat",
    "turn on": "aç",
    "turn out": "boşaltmak",
    "turn over": "devir",
    "turn round": "döndürmek",
    "turn to": "çevirmek",
    "turn up": "açmak",
    "TV": "televizyon",
    "twelfth": "onikinci",
    "twelve": "on iki",
    "twentieth": "yirminci",
    "twenty": "yirmi",
    "twice": "iki defa",
    "twin": "ikiz",
    "twist": "büküm",
    "twisted": "bükülmüş",
    "two": "iki",
    "type": "tip",
    "typical": "tipik",
    "typically": "tipik",
    "tyre": "tekerlek",
    "the unemployed": "işsiz",
    "the unexpected": "beklenmeyen",
    "ugly": "çirkin",
    "ultimate": "nihai",
    "ultimately": "en sonunda",
    "umbrella": "şemsiye",
    "unable": "aciz",
    "unacceptable": "kabul edilemez",
    "uncertain": "belirsiz",
    "uncle": "amca dayı",
    "uncomfortable": "rahatsız",
    "unconscious": "bilinçsiz",
    "uncontrolled": "kontrolsüz",
    "under": "altında",
    "underground": "yeraltı",
    "underneath": "altında",
    "understand": "anlama",
    "understanding": "anlayış",
    "underwater": "su altı",
    "underwear": "iç çamaşırı",
    "undo": "geri alma",
    "unemployed": "işsiz",
    "unemployment": "işsizlik",
    "unexpected": "beklenmedik",
    "unexpectedly": "beklenmedik",
    "unfair": "haksız",
    "unfairly": "insafsızca",
    "unfortunate": "şanssız",
    "unfortunately": "ne yazık ki",
    "unfriendly": "düşmanca",
    "unhappy": "mutsuz",
    "uniform": "üniforma",
    "unimportant": "önemsiz",
    "union": "Birlik",
    "unique": "benzersiz",
    "unit": "birim",
    "unite": "unite",
    "united": "Birleşik",
    "universe": "Evren",
    "university": "Üniversite",
    "unkind": "nezaketsiz",
    "unknown": "Bilinmeyen",
    "unless": "olmadıkça",
    "unlike": "aksine",
    "unlikely": "olası olmayan",
    "unload": "boşaltma",
    "unlucky": "şanssız",
    "unnecessary": "gereksiz",
    "unpleasant": "hoş olmayan",
    "unreasonable": "mantıksız",
    "unsteady": "kararsız",
    "unsuccessful": "başarısız",
    "untidy": "Düzensiz",
    "until": "a kadar",
    "unusual": "olağandışı",
    "unusually": "alışılmadık",
    "unwilling": "isteksiz",
    "unwillingly": "istemeyerek",
    "up": "yukarı",
    "upon": "üzerine",
    "upper": "üst",
    "upset": "üzgün",
    "upsetting": "üzücü",
    "upside down": "Tepe taklak",
    "upstairs": "üst katta",
    "upward": "yukarı",
    "upwards": "yukarı",
    "urban": "kentsel",
    "urge": "dürtü",
    "urgent": "acil",
    "us": "bize",
    "use": "kullanım",
    "used 1": "1 kullanılan",
    "used 2": "2 kullanılan",
    "used to": "alışığım",
    "useful": "işe yarar",
    "useless": "Faydasız",
    "user": "kullanıcı",
    "use up": "tüketmek",
    "usual": "olağan",
    "usually": "genelde",
    "vacation": "tatil",
    "valid": "geçerli",
    "valley": "vadi",
    "valuable": "değerli",
    "value": "değer",
    "van": "kamyonet",
    "variation": "varyasyon",
    "varied": "çeşitli",
    "variety": "Çeşitlilik",
    "various": "çeşitli",
    "vary": "farklılık göstermek",
    "vast": "Muazzam",
    "vegetable": "sebze",
    "vehicle": "araç",
    "venture": "girişim",
    "version": "versiyon",
    "vertical": "dikey",
    "very": "çok",
    "via": "üzerinden",
    "victim": "kurban",
    "victory": "zafer",
    "video": "video",
    "view": "görünüm",
    "village": "köy",
    "violence": "şiddet",
    "violent": "şiddetli",
    "violently": "şiddetle",
    "virtually": "fiilen",
    "virus": "virüs",
    "visible": "gözle görülür",
    "vision": "vizyon",
    "visit": "ziyaret etmek",
    "visitor": "ziyaretçi",
    "vital": "hayati",
    "vocabulary": "kelime hazinesi",
    "voice": "ses",
    "volume": "Ses",
    "vote": "oy",
    "wage": "ücret",
    "waist": "bel",
    "wait": "Bekle",
    "waiter": "Garson",
    "wake": "uyanmak",
    "walk": "yürümek",
    "walking": "yürüme",
    "walk out": "çıkmak",
    "walk up": "yanaşmak",
    "wall": "duvar",
    "wallet": "cüzdan",
    "wander": "gezmek",
    "want": "istemek",
    "war": "savaş",
    "warm": "Ilık, hafif sıcak",
    "warmth": "sıcaklık",
    "warm up": "ısınmak",
    "warn": "uyarmak",
    "warning": "uyarı",
    "wash": "yıkama",
    "wash away": "aşındırmak",
    "washing": "yıkama",
    "wash off": "yıkayıp temizlemek",
    "wash out": "feshetmek",
    "wash up": "yıkamak",
    "waste": "atık",
    "watch": "izlemek",
    "watch out": "dikkat et",
    "watch out for": "dikkat et",
    "water": "Su",
    "wave": "dalga",
    "way": "yol",
    "we": "Biz",
    "weak": "güçsüz",
    "weakness": "zayıflık",
    "wealth": "servet",
    "weapon": "silah",
    "wear": "giyinmek",
    "wear away": "yıpratmak",
    "wear off": "aşınmak",
    "wear out": "tüketmek",
    "weather": "hava",
    "web": "ağ",
    "website": "İnternet sitesi",
    "wedding": "düğün",
    "Wednesday": "Çarşamba",
    "week": "hafta",
    "weekend": "hafta sonu",
    "weekly": "haftalık",
    "weigh": "tartmak",
    "weight": "ağırlık",
    "welcome": "Hoşgeldiniz",
    "well": "iyi",
    "well known": "iyi bilinen",
    "west": "batı",
    "western": "batı",
    "wet": "ıslak",
    "what": "ne",
    "whatever": "her neyse",
    "wheel": "tekerlek",
    "when": "ne zaman",
    "whenever": "her ne zaman",
    "where": "nerede",
    "whereas": "buna karşılık",
    "wherever": "her nerede",
    "whether": "olup olmadığını",
    "which": "hangi",
    "while": "süre",
    "whisper": "fısıltı",
    "whistle": "ıslık",
    "white": "beyaz",
    "who": "DSÖ",
    "whoever": "her kim",
    "whole": "bütün",
    "whom": "kime",
    "whose": "kimin",
    "why": "neden",
    "wide": "geniş",
    "widely": "geniş ölçüde",
    "width": "Genişlik",
    "wife": "kadın eş",
    "wild": "vahşi",
    "wildly": "çılgınca",
    "will": "niyet",
    "willing": "istekli",
    "willingly": "isteyerek",
    "willingness": "isteklilik",
    "win": "kazanmak",
    "wind 1": "1 rüzgar",
    "wind 2": "2 rüzgar",
    "window": "pencere",
    "wine": "şarap",
    "wing": "kanat",
    "winner": "kazanan",
    "winning": "kazanan",
    "winter": "kış",
    "wire": "tel",
    "wise": "bilge",
    "wish": "dilek",
    "with": "ile",
    "withdraw": "Çekil",
    "within": "içinde",
    "without": "olmadan",
    "witness": "tanık",
    "woman": "Kadın",
    "wonder": "merak etmek",
    "wonderful": "olağanüstü",
    "wood": "Odun",
    "wooden": "ahşap",
    "wool": "yün",
    "word": "sözcük",
    "work": "iş",
    "worker": "çalışan",
    "working": "Çalışma",
    "work out": "egzersiz yapmak",
    "world": "Dünya",
    "worried": "endişeli",
    "worry": "endişelenmek",
    "worrying": "endişe verici",
    "worse": "daha da kötüsü",
    "worship": "ibadet",
    "worst": "en kötü",
    "worth": "değer",
    "would": "Would",
    "wound 1": "yara 1",
    "wounded": "yaralı",
    "wrap": "paketlemek",
    "wrapping": "sarma",
    "wrist": "bilek",
    "write": "yazmak",
    "write back": "cevap yazmak",
    "write down": "yazmak",
    "writer": "yazar",
    "writing": "yazı",
    "written": "yazılı",
    "wrong": "yanlış",
    "wrongly": "yanlış",
    "yard": "avlu",
    "yawn": "esnemek",
    "yeah": "Evet",
    "year": "yıl",
    "yellow": "Sarı",
    "yes": "Evet",
    "yesterday": "dün",
    "yet": "hala",
    "you": "sen",
    "young": "genç",
    "your": "sizin",
    "yours": "seninki",
    "yourself": "kendin",
    "youth": "gençlik",
    "zero": "sıfır",
    "zone": "bölge"
  }

    async function searchDictionary() {
    const wordInput = document.getElementById('kelime').value;
    const definition = dictionary[wordInput.toLowerCase()];
  
    // if the input is empty clear the box
    if (!wordInput.trim()) {
      clearWordList();
      return;
    }
  
    const wordListContainer = document.getElementById('wordList');
  
    // Clear previous entries
    clearWordList();

    if (definition) {
        const wordEntry = document.createElement('div');
        wordEntry.classList.add('word');
        wordEntry.innerHTML = `<h2 style="color: red;">${wordInput}</h2><hr><p><strong>Definition:</strong> ${definition}</p>`;
        wordListContainer.appendChild(wordEntry);
      } else {
        const notFoundEntry = document.createElement('div');
        notFoundEntry.classList.add('word');
        notFoundEntry.innerHTML = `<h2 style="color: red;">${wordInput}</h2><p>Word not found.</p>`;
        wordListContainer.appendChild(notFoundEntry);
      }
    }

    function clearWordList() {
        const wordListContainer = document.getElementById('wordList');
        wordListContainer.innerHTML = "";
    }

///////////////////////Mini Quiz////////////////////////////////////////////////

var currentScore = 0;

function generateQuiz() {
    // Select a random word
    var words = Object.keys(dictionary);
    var randomWord = words[Math.floor(Math.random() * words.length)];
    var correctAnswer = dictionary[randomWord];
    
    // Collect all answers in an array
    var allAnswers = [correctAnswer];
    while (allAnswers.length < 4) {
        var randomAnswer = words[Math.floor(Math.random() * words.length)];
        if (!allAnswers.includes(dictionary[randomAnswer])) {
            allAnswers.push(dictionary[randomAnswer]);
        }
    }
    
    // Shuffle the elements in the array using a random sorting mechanism
    allAnswers.sort(function() {
      // Generate a random value between -0.5 and 0.5
      //   - If the result is negative, the elements are sorted with the first element appearing before the second.
      //   - If the result is positive, the order is reversed.
      //   - If the result is exactly 0, the order remains unchanged.
      return 0.5 - Math.random();
    });

    // Add to the HTML page
    var questionElement = document.getElementById('question');
    var optionsContainer = document.getElementById('options-container');
    
    questionElement.textContent = "What is the Turkish meaning of the word '" + randomWord + "'?";
    
    optionsContainer.innerHTML = ""; // Clear
    
    allAnswers.forEach(function(answer) {
        var button = document.createElement('button');
        button.textContent = answer;
        button.classList.add('quiz-button'); // Add the class to the button
        button.addEventListener('click', function() {
            handleAnswer(answer, correctAnswer, randomWord);
        });
        optionsContainer.appendChild(button);
    });
}

function handleAnswer(selectedAnswer, correctAnswer, randomWord) {
  var scoreElement = document.getElementById('current-score');

  var feedbackElement = document.getElementById('last-word-feedback');
  feedbackElement.textContent = ""; // Clear previous feedback

    if (selectedAnswer === correctAnswer) {
      feedbackElement.textContent = "Correct!";
      currentScore++;
    } else {
      feedbackElement.textContent = "Incorrect, ' " + randomWord + " = " + correctAnswer + " '.";
      currentScore--;
    }
    
  scoreElement.textContent = currentScore;
  generateQuiz(); // Generate a new quiz
}

// Generate the initial quiz
generateQuiz();


///////////////////////Random word decider//////////////////////////////////////

function getRandomWord() {
  // Get the container element with id 'randomWordContainer'
  const randomWordListContainer = document.getElementById('randomWordContainer');

  // Clear any previous entries in the container
  randomWordListContainer.innerHTML = "";

  try {
    // Get an array of word keys from the 'dictionary' object
    const wordKeys = Object.keys(dictionary);

    // Get a random index within the range of the wordKeys array
    const randomIndex = Math.floor(Math.random() * wordKeys.length);

    // Retrieve the random word and its definition from the 'dictionary' object
    const randomWord = wordKeys[randomIndex];
    const randomDefinition = dictionary[randomWord];

    // Create a new div element to display the random word and definition
    const randomWordEntry = document.createElement('div');

    // Set the inner HTML of the div with the random word and definition
    randomWordEntry.innerHTML = `<h2 style="color: blue;">Random Word:</h2><p><strong>${randomWord}</strong></p><hr><p><strong>Definition:</strong> ${randomDefinition}</p>`;

    // Append the div element to the 'randomWordContainer'
    randomWordListContainer.appendChild(randomWordEntry);
  } catch (error) {
    // Log an error message to the console if there's an error
    console.error('Error getting random word:', error.message);
  }
}
